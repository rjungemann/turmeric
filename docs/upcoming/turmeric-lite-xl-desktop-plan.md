...
### Phase 5 -- Distribution: "Turmeric Studio" branded bundle (1 week)
...
theme so a brand-new user does one download.

- Vendor Lite XL upstream into `vendor/lite-xl/` at a pinned tag.
- Bake `turmeric.lua` + `colors/turmeric.lua` + a small `init.lua`
  default into the bundle's `Resources/` (macOS) /
  `data/` (Linux/Windows) so they ship enabled.
- Rebrand: `Info.plist`/icon on macOS, `.rc` resources on Windows,
  `.desktop` entry on Linux. The Lite XL window title is plugin-driven,
  so the in-app branding is one line of Lua.
...
- Distribute as `TurmericStudio-<version>-macos-arm64.dmg`,
  `-universal.dmg`, `-windows-x86_64.zip`, `-linux-x86_64.AppImage`,
  released alongside `tur` itself.
...
- A `tools/lite-xl/install.sh` (and `.ps1`) script for users who
  already have stock Lite XL and just want the Turmeric plugin.
- **Homebrew cask** for the macOS bundle, distributed via a tap, with
  `depends_on formula: "turmeric"` so a single command pulls both
  editor and compiler.

Acceptance: a brand-new user downloads the macOS bundle, opens it,
...
Phase 5 bundled distro adds ~5 working days (vendor drop, branding,
Homebrew cask, three-OS CI tweaks, signing). Total with bundled
distribution: ~12 working days.
