...
| ID | Task |
|----|------|
| TM0-1 | Add `Hash[A]` typeclass and built-in instances for `int`, `float`, `cstr`, `bool` |
| TM0-2 | Define `Map[K V]` struct wrapping the HAMT `ptr<void>` |
| TM0-3 | Implement `tmap-new`, `tmap-assoc`, `tmap-dissoc`, `tmap-count`, `tmap-merge` |
| TM0-4 | Implement `tmap-get` returning `Option[V]` (depends on TC1-option or a stub) |
| TM0-5 | Implement `tmap-has?` |
| TM0-6 | Add constrained `Eq[Map[K V]]` instance using `Hash[K]` + `Eq[K]` + `Eq[V]` |
| TM0-7 | Deprecation notice on untyped `map.tur` (`assoc`, `dissoc`, `get`, `has?`, `count`, `merge`) -- keep for one release cycle |
| TM0-8 | Update `gendocs.py` / doc tooling to render type parameters in `defstruct` |
| TM0-9 | Add fixtures in `tests/fixtures/` covering: empty map, single assoc, dissoc, merge, collision (two keys with same hash), `tmap-eq?` |
...
Tasks: TC1-1 through TC1-5 (struct, push, get, pop, `Eq[Vec[A]]` instance).
...
Tasks: TC1-6 through TC1-9 (type alias, constructors, `Eq[List[A]]` instance).
...
Tasks: TC1-13 through TC1-17 (struct, constructors, `map-option`, `Eq[Option[A]]`).
...
Tasks: TC1-18 through TC1-22 (struct, constructors, `map-result`, `Eq[Result[A B]]`).
...
Tasks: TC1-23 through TC1-25 (struct, accessors, `Eq[Pair[A B]]` instance).
