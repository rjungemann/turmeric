...
| `math` | FAIL | extern-c `malloc` redeclare | `math__vec3.c:17`, `math__vec4.c:15`, `math__mat4.c`, `math__quat.c`, `math__vec2.c` -- `(extern-c malloc [^int size] :int)` conflicts with the system `<stdlib.h>` prototype. Not strictly a typing-wave regression but blocks math's CI; replace with an inline-C `#include <stdlib.h>` block. |
