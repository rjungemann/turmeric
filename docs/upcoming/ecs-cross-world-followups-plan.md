...
| GEN-V0.1 | ~~`defworld` macro in a new module `ecs/world.tur`; emits cid constants + box/load/free + registration call.~~ **Shipped** as two coordinated macros instead of one: `defcomponent-cids` (cid constants, `ecs/world`) pairs with the existing `defworld-box-helpers` (box/load/free, `ecs/xworld`, shipped earlier under the same plan tag). Per-process registration deferred -- the `tur-ecs/inspect` surface that would consume it does not exist yet, so wiring a call to nothing was dropped. | `../turmeric-spices/spices/ecs/src/ecs/world.tur` |
| GEN-V0.2 | Tests: two `defcomponent-cids` blocks in one TU number independently from 0 (matches the `(world, cid)` conflict-key discipline). **Shipped.** | `../turmeric-spices/spices/ecs/tests/defcomponent-cids.tur` |
| GEN-V0.3 | Guide update: the "Plumbing" subsection now shows `defcomponent-cids` as the canonical form alongside `defworld-box-helpers`. **Shipped.** | `docs/guides/ecs-guide.md` |
