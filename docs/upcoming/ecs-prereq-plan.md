...
| **H** | [typeclass-constrained-defn-rejected.md](../reported/typeclass-constrained-defn-rejected.md) | **Items 1 + 2 fixed 2026-06-11; item 3 still open.** Item 1: defn parser accepts the `(defn name [TypeVars] [(Class V) ...] [params] :ret)` shape, registers constraints, and end-to-end works for int-carried receivers; covered by `tests/fixtures/defn-class-constraint-list-syntax/`. Item 2 fixed earlier this session. Item 3 (carrier-int dispatch on struct receivers) still blocks polymorphic wrappers over by-value struct worlds. The ECS spice's monomorphic-dispatch HasComponent surface already works; polymorphic wrappers work for int-carried worlds; struct-receiver polymorphism awaits item 3. | ECS HasComponent classes (monomorphic SHIPPED; polymorphic over int worlds SHIPPED; polymorphic over struct worlds blocked); any world-polymorphic library |
