...
| **F** | [top-level-def-init-dropped.md](../reported/top-level-def-init-dropped.md) | **Open** -- silent miscompile. Discovered 2026-06-11 while writing E2 stage scheduler tests. `(def y 7)` at top level emits `static int64_t y_888;` but never initializes it; reads return zero-bytes. Workaround: `(defn pos-cid [] : int 0)` zero-arg defns in place of `def`-constants. Audit recommended on every `def` site in `stdlib/` -- callers may be silently reading 0 in production. | Every top-level `(def name value)` declaration; potentially `stdlib/math.tur::PI`, `stdlib/reactor.tur::READ/WRITE/...`, `stdlib/schema.tur::SCHEMA_*` |
