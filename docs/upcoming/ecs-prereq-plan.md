...
|---|---|---|---|
| **A** | [ecs-macro-symbol-synthesis-missing.md](../reported/ecs-macro-symbol-synthesis-missing.md) | **Fixed** | Per-component accessors, `defsystem` name-keyed metadata, `HasComponent` classes |
...
```
   A (str->sym)        B (return-type)        C (backquote dot-sym)        D (macro inline-C)
   ───────────         ───────────────        ─────────────────────        ──────────────────
   ✅ shipped          ✅ shipped              ✅ shipped                    open, low priority
```

**No compiler prerequisite blocks the ECS plan anymore.** The
2026-06-11 research narrowed gap D from "blocks parallel E2" to
"surface-syntax pothole in one specific macro shape" -- see
[macro-cannot-emit-inline-c-block.md](../reported/macro-cannot-emit-inline-c-block.md)
§ "What the ECS plan actually needs" for the load-bearing finding:

> Turmeric already implicitly coerces a top-level defn name to a
> `ptr<void>` at call sites that expect one.

This means `defsystem` can emit `(make-system reads writes ~impl-name)`
with `~impl-name` referencing a peer top-level defn, and the compiler
fills in the function pointer automatically -- no inline-C trampoline,
no per-system glue, no FFI work.

**Stop-or-go gates (final):**

- **A, B, C are in.** The ECS plan's `defworld` accessor generation,
  `HasComponent` classes, `defsystem` name-keyed metadata, and clean
  `dense-get` / `sparse-get` (no witness arg) are all implementable
  against the substrate as it stands today.
- **D is no longer a gate.** The parallel-on-threadpool path
  for E2 uses the implicit fn->ptr<void> coercion, which is already
  shipping. D can land any time as ergonomic polish.
...
3. **Resume the ECS spice's E2 work** -- the unblocked path.
   `defworld` accessor generation, `HasComponent` classes,
   `defsystem` name-keyed metadata are all routine macros against
   the substrate as it stands. The macro-emitted `defsystem`
   leverages the implicit `defn-name -> ptr<void>` coercion to
   thread the system body through to `make-system` without any
   inline-C trampoline.
