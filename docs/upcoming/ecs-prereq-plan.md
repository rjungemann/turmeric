...
| **H** | [typeclass-constrained-defn-rejected.md](../reported/typeclass-constrained-defn-rejected.md) | **Open** -- discovered 2026-06-11 while attempting E2 HasComponent classes. Three separable items: (i) `[W] [(HasX W)] [w : W]` defn-constraint syntax is rejected by the parser; (ii) typed `[w : W]` method params SEGV in `elab_method_call`; (iii) carrier-int dispatch passes `int64_t` where instance expects struct. Blocks the v1 OPTIONAL "typeclass-bounded systems" path; v1 DEFAULT monomorphic path is shipped. | ECS HasComponent classes (plan's typeclass-bounded systems path); any world-polymorphic library |
