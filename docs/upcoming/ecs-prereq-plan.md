...
| **G** | [macro-unquote-in-type-position-rejected.md](../reported/macro-unquote-in-type-position-rejected.md) | **Open** -- discovered 2026-06-11 while writing the `defworld` per-component accessor emit. `(defn name [w : ~WName] : ~TName ...)` inside a backquoted macro body fails to expand because the type-position parser sees the unquote form. Without it, the macro can mint the *name* of `get-Pos` but can't type the body over `Pos`. | `defworld` per-component accessors; any macro-emitted typed defn parameterized over a type arg |
