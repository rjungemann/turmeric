...
| **H** | [typeclass-constrained-defn-rejected.md](../reported/typeclass-constrained-defn-rejected.md) | **Item 2 fixed 2026-06-11; items 1 + 3 still open.** Item 2 (typed-method-param SEGV) now bails cleanly via a null-def check at `elab_typeclasses.c:3229`. Items 1 (constraint syntax) and 3 (carrier-int dispatch on struct) still gate the polymorphic-wrapper path. The ECS spice ships a monomorphic-dispatch HasComponent surface (`defcomponent-class` + `defcomponent-class-instance`) that uses untyped `[^borrow w]` methods to sidestep both. Regressions: `tests/fixtures/errors/typeclass-typed-method-param-null-def-safe/` (compiler) and `tests/has-component-class.tur` (spice). | ECS HasComponent classes (monomorphic path SHIPPED; polymorphic path blocked); any world-polymorphic library |
