...
- [x] Fix `tests/fixtures/scheduler-multithread/` — implemented Option B: `(load "path")` is now a source-file inclusion preprocessor in `src/elab.c::elaborate_program`. Top-level forms are scanned for `(load <string>)`; matched files are read, parsed via `read_all`, and their forms spliced into the work list before two-pass elab. Visited paths are tracked (interned Symbol pointers) to skip duplicate loads, and the loop iterates until fixed point so loaded files may themselves load others. Stub `elab_load` returns `EX_NIL_LIT` (loads are pre-expanded). Also: fixed 6 missing `(` parens in `stdlib/fiber.tur`, removed an obsolete duplicate slot-based `TurTimerWheel` block in `src/emit.c`, and added a minimal multi-threaded scheduler runtime (`TurSchedulerMT` + `tur_scheduler_mt_*`) to the emit.c preamble — a shared-queue pthread pool sufficient for the fixture's needs (real work-stealing remains in host-side `scheduler.c`).
  - **Acceptance:** `tests/fixtures/scheduler-multithread/` PASSES; binary prints two distinct `pthread_self()` thread IDs.
