...
**Status:** Planned (v2). Prerequisites: Phase 19 (Algebraic effects), T19 (Thread primitives — `Mutex<T>`, condition variables, `Arc<T>`). See [stm-plan.md](stm-plan.md) for full design, prerequisites checklist, and task breakdown.
...
**Status:** Planned (v2). Prerequisites: Phase 15 (Typeclasses — `Eq`/`Hash` dispatch needed for key comparison), Phase 19 (Algebraic effects — `@ {Unsafe}` tracks internal raw memory use in `hamt.c`). See [hamt-plan.md](hamt-plan.md) for full design, prerequisites checklist, and task breakdown.
...
See [module-system-plan.md](module-system-plan.md) for the full task breakdown (Phases R0–R8).
