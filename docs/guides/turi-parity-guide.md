...
- **A peer written with `async` now runs on both backends.** It used to run here
  and deadlock compiled: the compiled session runtime blocks an OS thread on a
  condvar, and compiled `async` ran the body on the spawner's own thread, so
  `(async (fn [] (recv ch)))` hung the binary while the interpreter's
  cooperative rendezvous yielded to its scheduler. Since 2026-09-17 the compiler
  gives such a body its own OS thread and joins it at the `await`, so the shape
  composes on both paths and the `TUR-W0043` warning that named the hang is
  retired:
