...
| **`float32`** generic call result -- ELABORATION, not a missing bridge: `A := float32` binds for the argument and is lost for the result, so the call types as `int` and prints the carrier (`7.1` -> `1088631603`); the float64 twin resolves concretely and is fine | [`float32-generic-call-result-printed-as-carrier`](https://github.com/rjungemann/turmeric/blob/main/docs/reported/float32-generic-call-result-printed-as-carrier.md) |
