...
| **`float32`** ascribed LITERAL in a mixed expression -- emitted as the bare double literal, so `(= (mono (:: 7.1 float32)) (:: 7.1 float32))` compares float against double and is false (turi: true) | [`float32-ascribed-literal-compares-as-double`](https://github.com/rjungemann/turmeric/blob/main/docs/reported/float32-ascribed-literal-compares-as-double.md) |
