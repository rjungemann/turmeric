...
- **Threads.** A program that starts threads (through the seam:
  `thread-spawn-fn`, `session-spawn`, a task group, a Turmeric module's own
  `pthread_create`) runs them in parallel, and its memory is reclaimed: each
  thread allocates from its own cache of slots, and a collection stops the
  other threads by signal wherever they are, the way the Boehm collector
  does, so nothing is compiled into the program's loops. The stop signal
  restarts the system call it interrupts; a program's own inline C that
  calls a blocking function the runtime does not know (the known ones are
  the joins, the condition waits, `nanosleep`, `poll`, `select`, `accept`,
  `connect`, `recv`, `read`, `waitpid`, `sem_wait`, `epoll_wait`, `kevent`)
  may see EINTR from it and should retry. A thread the program did not
  start (a library's own, calling back in) stops with the reason at its
  first allocation. `TUR_R7RS_GC=0 tur build prog.tur`, or `tur
  --no-r7rs-gc build prog.tur`, still builds without the collector; the
  data then stays allocated until the process exits, the way a Turmeric
  `:heap` box does.
