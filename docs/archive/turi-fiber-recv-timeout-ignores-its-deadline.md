> **Resolved 2026-09-16** (turi-session-expansion-plan, S1). The fiber arm of
> `session_recv_timeout` (`src/turi/eval.c`) now arms a timer future on the
> remaining time alongside the channel's `recv_waiter`, so either the deposit
> or the deadline resumes the fiber; `session_wake` and `flush_wakers` both
> refuse a fiber that is no longer SUSPENDED so the two wake sources cannot
> double-enqueue it. Pinned by `tests/fixtures/session-timeout-fiber-turi`.
