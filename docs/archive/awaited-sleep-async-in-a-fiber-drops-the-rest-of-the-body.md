> **Resolved 2026-09-16** (turi-session-expansion-plan, alongside S1). The
> fiber arm of `native_sleep_async` (`src/turi/fiber.c`) returns the resolved
> timer future instead of nil, so the surrounding `await` passes through and
> the fiber body continues. Pinned by
