...
| ~~G8~~ | site 8, nested-RECEIVER case | method dispatch where the receiver is **itself a re-resolved class-method call** (`(join (join x y) y)`): the receiver's elaborated type is the representative instance's carrier, so `emit_dispatch_tyvar` saw a concrete `int` and the OUTER call kept `__inst_C_m_int` while the inner one retargeted -- a silent wrong answer (7 for 7.1). Recovered by re-resolving the receiver when the inner method's declared return is the bare class variable | `emit_reresolve_disp_type` (recursive) | **[FIXED]** 2026-09-11 | `nested-class-method-call-picks-the-first-instance` |
