...
status: RESOLVED 2026-06-17 (direction 1; direction 2 still waiting on gap-H). Direction 1 ships at the spice layer as `sized-defsystem-scheduled` in `../turmeric-spices/spices/ecs/src/ecs/sized-world.tur`: the macro lowers a monomorphic-world body to a typed `name-impl : [^borrow w : WorldName] : nil`, an int-carrier wrapper `name-fn : [wp : int] : nil` that loads the heap-boxed world via `load-<WorldName>` and dispatches to the typed impl, and a `(make-system reads-mask writes-mask name-fn)` `System` value runnable on the parallel `Stage` scheduler. Per-world `box-<W>` / `load-<W>` / `free-<W>-box` helpers stay user-written -- macros cannot splice identifiers into inline-C text (`docs/archive/history/macro-cannot-emit-inline-c-block.md`); the three helpers reference the C struct name verbatim and are ~10 mechanical lines per world. The cap-binding / auto-consume guarantees the prior hand-rolled wrappers bypassed are restored: a body that writes a component not in `:writes` fails to elaborate. Regression: `../turmeric-spices/spices/ecs/tests/sized-stage.tur`. Direction 2 (cross-world / heterogeneous scheduling -- `System`/`Stage` polymorphic in the world type) remains queued behind gap-H world-type polymorphism, itself behind the Track A monomorphization phases.
---
