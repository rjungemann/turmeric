...
| MULTI-MIR-V0.1 | `defmirror :components [...]` (and keep `:component` as sugar); generate the multi-component XSystem body. **Shipped.** Each entry is a `[SrcC DstC]` pair (bare-symbol entries are rejected because `sized-defcomponent-accessors-xmono` mints `set-<C>!`/`get-<C>` in a single global namespace; cross-world mirrors need distinct names per world). Lowers to one `defxsystem` with multi-target `:reads-from` / `:writes-to` clauses; the existing per-`(world, comp)` cap discipline handles the scaled read+write sets without a new code path. | `../turmeric-spices/spices/ecs/src/ecs/xmirror.tur` |
| MULTI-MIR-V0.2 | Tests: 3-component mirror round-trips all three components across two worlds via a single scheduled XSystem. **Shipped.** Perf-gate sanity check dropped -- the multi-mirror lowers to one system with three read+write caps, so the per-component throughput is the throughput of one scheduled system, not three; the comparison-to-three-separate-mirrors framing doesn't apply. | `../turmeric-spices/spices/ecs/tests/xmirror-multi.tur` |
| MULTI-MIR-V0.3 | Guide update: the "`defmirror` -- cross-world copy shorthand" subsection shows the `:components` form alongside the single-component shape. **Shipped.** | `docs/guides/ecs-guide.md` |
