...
status: RESOLVED 2026-06-13 as Prereq 5. The root cause turned out to be two issues compounding: (a) `parse_typeclass_method` in `elab_typeclasses.c:881/891` resolved return-type forms without passing the class's type parameters, so a class tyvar nested inside `(Result a cstr)` was parsed as an undefined opaque rather than TY_TYVAR; (b) the return-dispatch resolution in `elab_try_return_dispatch` then set the call's elab type to the bare unified `bound` instead of substituting `a := bound` into the method's full return type. Fixed by threading `class_type_params` through the two `type_expr_from_form` calls (with a NULL-tolerant `type_param_kinds` fallback in `elab_types.c:403`) and by changing the result type at `elab_typeclasses.c:3085` to use `*e->expected_type` (the ascribed type, which already substitutes `a := bound`) when an ascription is present. Regression fixture: `tests/fixtures/typeclass-return-dispatch-result-wrapped/`. The spice-side typed `Decode` typeclass surface in `../turmeric-spices/spices/json` is now unblocked.
---
