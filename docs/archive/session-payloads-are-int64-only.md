> **Resolved 2026-09-16** (turi-session-expansion-plan, S3.5). The send
> operand and the recv result are now lowered by payload class
> (`session_payload_to_word` / `session_payload_from_word`,
> `src/compiler/elab_sessions.c`): floats are bit-reinterpreted through
> `tur_session_f64_bits` / `tur_session_bits_f64` (fix direction 1), pointers
> cast through `intptr_t` (direction 2), and a by-value struct/ADT is rejected
> at elaboration with `TUR-E0212` naming the type (directions 3 and 4 -- send it
> behind `rc<T>`/`ref<T>` or a `:heap` record). Pinned by
> `tests/fixtures/session-payload-float`, `session-payload-float-timeout`,
> `session-payload-cstr`, `session-delegation-over-channel`, and
> `tests/fixtures/errors/session-payload-struct`.
