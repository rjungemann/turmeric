...
> **Status:** Phases G0–G4 substantially complete; `equal-cong` deferred until HKT available; polish items remain (benchmarks, `tur explain` codes, fuzz tests)
...
|---|---|---|---|
| G0 | Plain ADTs — `defdata` + `match` | Sum types with exhaustiveness checking; tagged-union C codegen | **Complete** |
| G1 | GADT syntax and AST | `defgadt` with constructor return-type annotations; no type refinement yet | **Complete** |
| G2 | Type refinement in `match` | Skolem equalities per arm; bidirectional checking propagates refinements | **Complete** |
| G3 | Type equality witnesses | Built-in `Equal` GADT; `coerce`; interaction with HRT skolems and HKT kinds | **Complete** (equal-cong deferred) |
| G4 | Integration & polish | Stdlib patterns, error messages, documentation, performance benchmarks | **Substantially complete** |
...
- [x] Phase 15 (Typeclasses, kind-`*`) is complete and stable
- [x] `TY_STRUCT` covers product types; a new `TY_ADT` can be added without conflict
- [x] `Expr` node enum in `src/expr.h` has space for `EX_MATCH`, `EX_DEFDATA`, `EX_DEFGADT`
- [x] `src/elab.c` has bidirectional checking (from HRT — HRT0–HRT2 complete) before G2
- [x] `src/types.h` can represent type equality constraints (implemented via named `TY_TYVAR` + `SkolemEnv` rather than a separate `TY_EQ_WITNESS` node)
- [x] Diagnostics (`src/diag.c`) can show type-level diffs with GADT skolem names
...
- [x] Recognize `defdata` as a type-definition keyword
  ```clojure
  (defdata Name [:copy]
    (Ctor1)
    ...
    ...)
  ```
- [x] Support type parameters: `(defdata Option [a] (None) (Some a))`
- [x] `[:copy]` modifier (same semantics as `defstruct :copy`) for copy-able ADTs
- [x] Recognize `match` as a special form:
  ```clojure
  (match scrutinee
    (Ctor1)          body1
    (Ctor2 x y)      body2
    _                default-body)
  ```
- [x] Nested patterns: `(match opt (Some (Some x)) x _ -1)`
- [x] Wildcard `_` and variable capture patterns
- [x] Guard clauses: `(Ctor x) when (> x 0)` — implemented in G4

#### AST extensions (`src/expr.h`, `src/types.h`)
- [x] Add `EX_DEFDATA` node: `{ name, type_params[], constructors[] }`
- [x] Add `EX_MATCH` node: `{ scrutinee, arms[] }` where each arm is `{ pattern, body }`
- [x] Add `TY_ADT` type kind: `{ AdtDef *def, Type *type_args[], n_type_args }`
- [x] Add `AdtDef` struct: name, type parameters, constructor descriptors
- [x] Add `CtorDef` struct: name, field types, parent `AdtDef`
- [x] Constructor names are globally registered as value-level functions in the symbol table

#### Exhaustiveness checking (`src/elab.c`)
- [x] Implement pattern coverage matrix: enumerate all constructors and mark covered cases
- [x] Error on non-exhaustive match: `non-exhaustive match: missing constructor 'Foo'`
- [ ] Warning on redundant arms (arm that can never be reached)
- [x] Wildcard `_` covers all remaining constructors

#### Type checking (`src/elab.c`)
- [x] Infer scrutinee type; look up `AdtDef` in the type environment
- [x] For each arm, bind the constructor's field names in the local environment
- [x] All arms must have the same result type (unification); emit "match arms have mismatched types" otherwise
- [x] Constructor call expressions elaborate like function calls: `(Some x)` has type `(Option (type-of x))`

#### Codegen (`src/emit.c`)
- [x] Emit a C tagged union: `struct tur_adt_Name { int tag; union { struct { ... } Ctor1; struct { ... } Ctor2; } as; }`
- [x] Constructor functions emit a struct initializer with the correct tag
- [x] `match` emits a `switch (x.tag)` with a `case` per constructor, binding field names to local variables
- [x] Integrate with existing copy/move/drop infrastructure (CopyKind propagation)
- [x] For copy ADTs, emit `tur_adt_Name_copy`; for move ADTs, ensure single-owner discipline

#### Memory management
- [x] ADT with all copy-able fields defaults to `CK_COPY`
- [x] ADT containing any move-only field is `CK_MOVE`
- [x] Recursive ADT fields (e.g., list nodes) must use `ref<T>` or `rc<T>` to avoid infinite-sized structs

### Fixtures
- [x] `adt-basic.tur` — `Color` enum, exhaustive match
- [x] `adt-param.tur` — `Option [a]`, `(option-map f (Some 3))`
- [x] `adt-nested.tur` — `(match (Some (Some 42)) (Some (Some n)) n ...)`
- [x] `adt-nonexhaustive.tur` — non-exhaustive match caught at compile time
- [x] `adt-copy.tur` — copy-able ADT assigned twice without error
- [x] `adt-move.tur` — move ADT triggers borrow error on second use
- [x] `adt-recursive.tur` — linked list `(defdata List [a] (Nil) (Cons a (ref (List a))))`

### Exit criterion
Sum types declared, constructed, and pattern-matched correctly; exhaustiveness enforced; C codegen compiles cleanly; copy/move semantics correct; all G0 fixtures green. **MET.**
...
- [x] Recognize `defgadt` keyword
  ...
  (defgadt Name [type-params...]
    ...
    ...)
  ```
- [x] The `: return-type` annotation is required on every constructor in a `defgadt`; missing annotations produce a parse error
- [x] `return-type` must be an application of `Name` to type arguments (validated in G2)
- [x] `defgadt` without `-Xgadt` flag produces an "unknown form" error with a hint

#### AST extensions (`src/expr.h`, `src/types.h`)
- [x] Add `EX_DEFGADT` node (separate from `EX_DEFDATA` for clarity)
- [x] Extend `CtorDef` with a `result_type` field: the explicitly annotated return type (a `Type *`)
- [x] Distinguish `AdtDef.is_gadt` flag: `false` for plain `defdata`, `true` for `defgadt`
- [ ] `rank()` helper (from HRT plan): report `TY_ADT` with GADT flag as requiring refinement

#### Pretty-printing (`src/types.c`, `src/diag.c`)
- [x] Print GADT constructor signatures including the `: return-type` in error messages
- [ ] In type mismatch errors, show which GADT constructor caused the refinement

#### Validation pass
- [x] Verify that each constructor's return type is an application of the parent GADT's type constructor
- [x] Verify that type arguments in the return type are either bound variables or concrete types
- [x] Detect and error on constructors whose return-type annotation does not mention the GADT name
- [ ] Kind-check type arguments in return-type annotations (default kind `*`)

### Fixtures
- [x] `gadt-syntax-basic.tur` — `defgadt` with return-type annotations parses
- [x] `gadt-syntax-multi.tur` — multiple type parameters in GADT
- [x] `gadt-syntax-error.tur` — missing annotation on a constructor in `defgadt` is rejected
- [x] `gadt-syntax-flag.tur` — `defgadt` without `-Xgadt` gives hint

### Exit criterion
GADT definitions parse; `EX_DEFGADT` nodes printed correctly in diagnostics; validation pass catches malformed constructors; no runtime codegen differences from G0 yet. **MET.**
...
- [x] In `match` elaboration, detect if the scrutinee has a `TY_ADT` with `is_gadt = true`
- [x] For each arm, call `elab_match_arm_gadt`:
...
- [x] Ensure the inferred type of each arm body is consistent with the overall match result type (which may itself be refined)
- [x] When the match result type contains a free GADT type variable and all arms refine it differently, use the most general common supertype (or error)

#### Skolem equality representation (`src/types.h`, `src/types.c`)
- [ ] Add `TY_SKOLEM_EQ` entry to `TypeKind`: a local equality assertion `a ~ T` in scope — implemented as `TY_TYVAR` with named vs. anonymous distinction rather than a separate kind
- [x] Store skolem equalities in a per-arm `SkolemEnv` (stack-allocated, freed on arm exit)
- [x] `type_equiv` and `type_unify` consult the current `SkolemEnv` when comparing types
- [x] Skolem equalities are *not* metavariables — they are rigid and non-backtrackable

#### Exhaustiveness for GADTs
- [x] Exhaustiveness checker must account for GADT constructor subsets: not all constructors are reachable for a given instantiation of the type parameter
- [x] Example: for scrutinee `(Expr int)`, `BoolLit` is unreachable — do not require it
- [x] Emit a *warning* (not an error) for arms that are unreachable given the scrutinee's type
- [x] Emit an error only when there is no arm covering a reachable constructor

#### Type variable scoping
- [x] A type variable refined in a GADT arm must not escape that arm's scope
- [x] Error: `skolem type variable 'a' (refined in 'IntTag' arm) escapes match body`
- [x] The scrutinee's free type variable is still in scope outside the match — only the refinement is arm-local

#### Codegen (`src/emit.c`)
- [x] GADT match codegen is identical to plain ADT match (`switch (tag)`)
- [x] No runtime representation difference — refinement is purely a compile-time property
- [x] The C type used in each arm may differ (e.g., `int64_t` vs `bool`) — emit appropriate local variable declarations per arm

### Fixtures
- [x] `gadt-refine-basic.tur` — `Witness` tag GADT; `show-witness` compiles and runs
- [x] `gadt-refine-expr.tur` — typed expression interpreter; `eval-expr` evaluates correctly
- [x] `gadt-refine-nat.tur` — `Nat` / `Vec n a` length-indexed vector; `safe-head` rejected on `VNil`
- [x] `gadt-refine-escape.tur` — skolem variable escaping arm caught at compile time
- [x] `gadt-refine-exhaustive.tur` — unreachable arm produces warning; missing reachable arm produces error
- [x] `gadt-refine-witness.tur` — HRT integration: named type variable `v :a` resolved per GADT arm (added during HRT integration work)

### Exit criterion
Type refinement propagates correctly through GADT match arms; `eval-expr` and `safe-head` compile and run; unreachable-arm warnings emitted; skolem escape caught; C codegen compiles cleanly; all G2 fixtures green. **MET.**
...
- [x] Define `Equal` in the standard library with compiler-magic support:
  ```clojure
  (defgadt Equal [a b]
    (Refl : (Equal a a)))
  ```
- [x] The `Refl` constructor is special: it forces `a = b` at the call site (unification under the current skolem environment)
- [x] Matching on `Refl` introduces `a ~ b` as a skolem equality in the arm body
- [x] `Equal` is recognized by the elaborator as the canonical equality witness type

#### `coerce` form (`src/forms.c`, `src/elab.c`)
- [x] Introduce `(coerce eq x)` where `eq : (Equal a b)` and `x : a`, producing `x : b`
- [x] Elaboration: check `eq` has type `(Equal a b)`, check `x` has type `a`, produce type `b`
- [x] Codegen: `coerce` is a no-op at runtime — identical memory layout for `a` and `b` in all cases
- [x] Error if `eq` is not an `Equal` value: `coerce requires an (Equal a b) proof as first argument`

#### `(~)` constraint notation (`src/reader.c`, `src/elab.c`)
- [x] Allow `(~ a b)` as a constraint in `defn` parameter lists:
  ```clojure
  (defn only-int [(~ a int) x :a] :int
    x)
  ```
  Note: `~` is the reader's unquote macro, so `(~ a T)` is parsed as `F_LIST[F_UNQUOTE(a), T]` — detection handles this accordingly.
- [x] Elaborate `(~ a b)` constraints as equality assumptions (not typeclass dictionaries)
- [x] Interaction with typeclass constraints: `(~ a int)` and `^Eq a` may coexist

#### Symmetry, transitivity, and congruence (`stdlib/equal.tur`)
- [x] `(defn equal-sym [eq : (Equal a b)] : (Equal b a) ...)`
- [x] `(defn equal-trans [eq1 : (Equal a b), eq2 : (Equal b c)] : (Equal a c) ...)`
- [ ] `(defn equal-cong [eq : (Equal a b)] : (Equal (f a) (f b)) ...)` — **deferred**: requires HKT kind-`* -> *` infrastructure

#### Interaction with HRT
- [x] An `(Equal a b)` proof inside a `forall` creates a rank-2 equality
- [x] Skolems from GADT refinement and skolems from `forall` (HRT) share the same rigid variable infrastructure
- [x] No conflict between HRT `∀-intro` skolems and GADT arm skolems (different scopes)
...
### Fixtures
- [x] `gadt-equal-refl.tur` — `Refl` used to prove `(Equal int int)`
- [x] `gadt-equal-coerce.tur` — `coerce` transports a value across a proof
- [x] `gadt-equal-sym.tur` — symmetry of `Equal`
- [x] `gadt-equal-trans.tur` — transitivity of `Equal`
- [x] `gadt-equal-constraint.tur` — `(~)` constraint in `defn` signature
- [x] `gadt-equal-error.tur` — `coerce` without valid proof rejected
- [x] `gadt-tilde.tur` — `(~ a int)` constraint in a function with named type variable parameter

### Exit criterion
`Equal` GADT usable; `coerce` is a type-safe zero-cost cast; `(~)` constraint notation works in `defn`; symmetry and transitivity stdlib functions compile and pass tests; all G3 fixtures green. **MET** (equal-cong deferred to G4 pending HKT).
...
#### Documentation
- [x] `docs/gadts-guide.md` — user-facing guide: ADT vs GADT, annotation syntax, common patterns, pitfalls
...
- [x] `stdlib/equal.tur` — `Equal` GADT, `coerce`, `equal-sym`, `equal-trans`; `equal-cong` deferred pending HKT
- [x] `stdlib/nat.tur` — type-level `Nat` via `defgadt`; `Zero`, `Succ`, `nat-to-int`, `nat-add`
- [ ] `stdlib/vec.tur` — length-indexed `Vec n a`; `safe-head`, `safe-tail`, `vzip`, `vmap` — `VNil`/`VCons` tested via fixture; dedicated stdlib file not yet written
- [ ] `stdlib/result.tur` — `Result e a` as a GADT (or plain ADT — TBD based on user need)

#### Error message improvements
- [x] GADT match exhaustiveness errors name the missing *reachable* constructors (not all constructors)
- [x] Skolem escape errors include which constructor arm introduced the variable
...
- [x] Add `when` guard syntax to `match` arms:
  ```clojure
  (match e
    ...
    ...)
  ```
- [x] Guards are elaborated as `bool` expressions in the arm's skolem environment
- [x] Exhaustiveness checker treats guarded arms as potentially incomplete
...
- [x] Integration tests: GADT + typeclasses + closures + defer + HRT (`gadt-integration.tur`)
- [x] Negative tests: skolem escape, non-exhaustive GADT match, `coerce` without proof
- [ ] Fuzz the type checker with randomly generated GADT definitions and match expressions

### Fixtures
- [x] `gadt-stdlib-nat.tur` — type-level `Nat` arithmetic compiles and evaluates
- [x] `gadt-stdlib-vec.tur` — `VNil`/`VCons` operations; `safe-head` rejects empty vector at compile time
- [x] `gadt-guard.tur` — `when` guard clauses in GADT match
- [x] `gadt-integration.tur` — GADT + HRT + typeclasses + closures + defer
- [x] `gadt-asan.tur` — no memory errors under ASan for GADT-heavy programs

### Exit criterion
All stdlib patterns compile and execute; documentation complete; performance benchmarks acceptable; `equal-cong` available once HKT lands; all G4 fixtures green. **Fixtures: MET. Remaining open items: `equal-cong` (awaiting HKT), performance benchmarks, fuzz tests, `tur explain` GADT codes, `stdlib/vec.tur` dedicated file.**
