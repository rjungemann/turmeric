...
> **Status:** OPEN (2026-06-05). Blocks Stage E of
>   `docs/upcoming/poly-closure-result-specialization-plan.md`. Stages B+C+D
>   landed for the dispatch-free shape; this report captures why the dispatch
>   shape (the actual `>>>` combinator) is deferred, and the retained TUR-E0705
>   guard that keeps it from silently miscompiling.
> **Severity:** expressiveness gap, guarded against miscompile. The float case
>   of a *dispatching* closure-returning generic is a hard error (TUR-E0705),
>   not a silent miscompile -- but it cannot yet be made to *work* the way the
>   dispatch-free case now does.
