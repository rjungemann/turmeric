...
status: OPEN. Filed 2026-06-12 while landing P2a minimal slice; the macro itself is correct (hand-writing the same form fails the same way). Workaround: keep instance bodies at ≤2 nested cons-chains, or split the body into a fixed-arity helper that receives flattened scalars instead of the struct.
---
