...
| --- | --- | --- |
| [sr2-carrier-seam-rotted](sr2-carrier-seam-rotted.md) | medium | `TUR_SR2_APP_SUM_BYVALUE=0`, the bisection hatch left behind when `parametric-sum-byvalue` graduated, no longer produces correct programs. Three defects: the hatch aborts the compiler unless `TUR_OPTION_NICHE=0` moves with it (the niche is layered on by-value sums, and nothing says so); `sum-passthrough-param-not-dropped` is a hard C compile error on the carrier path regardless of the niche; `option-niche-vec-closure-cmp` is a silent wrong answer only on the both-off diagonal, which is how it hid from two one-axis harnesses. Default path unaffected. Found by restoring `tests/run-sr2-seam.sh`, retired at graduation |
