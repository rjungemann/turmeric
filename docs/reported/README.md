...
| [msgpack-spice-workarounds-to-remove](msgpack-spice-workarounds-to-remove.md) | low | Follow-up to the four rows above, all of which are fixed and archived. `turmeric-spices/spices/msgpack` still carries the code that existed only to route around them: four `DecodeMp` primitive instances hand-written as carrier-shaped inline C, two `__mp-arr-*` helpers keeping `unwrap` out of a generic body, and eight `(:: ... Buf)` wrappers on pinned `encode-mp @Cons` call sites. Dead scaffolding -- nothing is wrong, but it teaches the next reader that the hazard is live. Could not be done with the fixes: that repo is a separate checkout and was absent from the session. `tests/container-round-trip.tur` is the whole verification |
