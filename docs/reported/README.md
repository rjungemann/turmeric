...
| [byvalue-recursive-adt-boxes-are-never-freed](byvalue-recursive-adt-boxes-are-never-freed.md) | low-medium | A self-recursive by-value `defdata` mallocs one box per link and frees none -- measured in plain Turmeric with no `any` anywhere: 3 cells / 3 allocations, 5 cells / 5 allocations, linear. The recursive `tl` field cannot ride the int64 carrier, so its slot holds a pointer to a heap copy; `needs_drop_glue` is set only for `rc`/`ref`/`weak` fields, so such a field is owning in the allocation sense and unnamed in the ownership model. Split out of the frame-box report above, which measured it while establishing it was NOT that bug. `tests/fixtures/saffron-higher-order` carries a `known-leak` pointing here (840 bytes / 21 allocations) -- Saffron makes the shape easy to reach, it does not create it. `AdtDef.is_self_recursive` already records the property; the open question is aliasing |
