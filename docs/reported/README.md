...
| [self-typed-heap-parametric-field-unsupported](self-typed-heap-parametric-field-unsupported.md) | medium | Filed 2026-09-19. `(defstruct Node :heap [A] (val A) (next (Node A)))` is rejected with `expected int, got int`, and with a typed terminator it overflowed `tur`'s own stack (fixed: `adt_app_is_byvalue_product` now guards re-entry through a self-typed field; the shape then stops at a clean repr ICE). It is the shape M7 and the `& rest : any` gap are gated on |
