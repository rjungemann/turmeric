...
| [rc-ref-conversion-and-weak-upgrade-leak](rc-ref-conversion-and-weak-upgrade-leak.md) | medium | `ref/from-rc` transfers the payload out of the control block and nothing frees it; `upgrade`'s `option<rc<T>>` leaks its box when crossed into a `ptr<void>`. Same carrier-crossing shape as the fixed `rc-of-adt-leaks-the-payload` |
