...
| --- | --- | --- |
| [serial-shift-non-capturing-lambda-receiver-rejected](serial-shift-non-capturing-lambda-receiver-rejected.md) | low | a `(fn [k] ...)` serial-shift receiver that captures nothing is neither a named receiver nor an EX_CLOSURE, so the context is rejected with TUR-E0706; name it or capture something |
