...
| [httpd-async-limit-asserts-a-racy-split](httpd-async-limit-asserts-a-racy-split.md) | low | `tests/fixtures/httpd-async-limit` asserts an exact `ok=2`/`busy=2` split that is a scheduling race, not a cap invariant -- a loaded runner can produce `ok=3`/`busy=1` (seen on macOS CI, run 30738162068). Test-only; overlaps [windows-httpd-async-limit-hangs-on-ci](windows-httpd-async-limit-hangs-on-ci.md)'s "Defect 1" (same racy gate, found independently via a Windows hang). Also flags that `handler-ran` is a hardcoded literal `2`, not measured, so the fixture cannot fail on the property it advertises |
