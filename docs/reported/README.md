...
| [jit-threaded-program-hangs-under-load](jit-threaded-program-hangs-under-load.md) | medium | Under CPU load, a threaded program run under `tur jit` sometimes spins forever (2 of 25 runs of `r7rs-threads-pause`); the compiled build never does. Suspected cause: c2mir does not implement `_Thread_local`, so the runtime's thread-locals are shared between threads |
