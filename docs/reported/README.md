...
| [any-narrowing-broken-for-parametric-receivers](any-narrowing-broken-for-parametric-receivers.md) | high | All four narrowing routes fail on an `any` holding a parametric value. `(is? x Option)` is **silently false**; `(cast x Option)` panics `cast: any holds Option, not Option`; the parameterised spellings are rejected by the reader. `emit_any_type_id` interns by `type_name` (per instantiation, deliberately) while a bare `Option` target resolves to the head -- two ids, both displayed `"Option"`. `type-of` answers `"Option"` throughout, so nothing on the surface shows what is wrong |
| [generic-fn-in-any-return-position-emits-uncompilable-c](generic-fn-in-any-return-position-emits-uncompilable-c.md) | medium | `(defn f [] : any (some 7.1))` emits a call to a bare `some` that was never instantiated -- implicit declaration returning `int`, then an incompatible assignment into the box. Writing `(Some 7.1)` directly works, so it is specific to a generic defn whose monomorph the widen site does not request |
| [typeclass-dispatch-on-any-receiver-emits-uncompilable-c](typeclass-dispatch-on-any-receiver-emits-uncompilable-c.md) | medium | When resolution pins an instance, nothing checks the receiver's *representation*, so an `any` box is handed to a concrete-typed instance function. Two symptoms, one cause: with exactly one instance the ambiguity guard never fires, and the `@TypeName` witness has the same hole -- so following the compiler's own hint ("use @TypeName syntax") produces a cc error. `is?` narrowing and `cast` both work, so the bridge exists and is just not applied at these two sites |
