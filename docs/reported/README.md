...
| [tur-init-help-scaffolds-and-overwrites](tur-init-help-scaffolds-and-overwrites.md) | **high** (data loss) | `tur init --help` does not print help: `--help` is unhandled in `cmd_init`, falls through to the project-name slot, and with no name the CWD's basename is used -- so in a checkout called `turmeric` it SCAFFOLDS. Scaffolding then overwrites a pre-existing `.gitignore` and `README.md` without refusing (the existing refusal is keyed on a MANIFEST being present, not on files about to be clobbered). Run here it destroyed 209 lines of `.gitignore`, which unmasked ~2000 test artifacts as untracked; both files were tracked so `git checkout` recovered them, but uncommitted edits would be gone. No test caught it: `run-tur-new.sh` and `run-sweet-manifest.sh` both scaffold inside `mktemp -d`, and nothing checks that `--help` prints help. Two small independent fixes |
