...
| [vec-any-monomorph-is-half-plumbed](vec-any-monomorph-is-half-plumbed.md) | medium | With the ICE fixed a `(Vec any)` builds, stores and frees correctly -- and is write-only: `vec-get` reports its result as `int`, so an element cannot be read back with its tag, and ascribing past it re-tags the carrier word instead (compiled says `int`, interpreted says `float` -- the divergence is how the shape announces itself). Identical on both back ends, so elaboration, not codegen; it is S6's actual subject, since the same question governs `#map{}`, `#set{}` and cons lists. Second half: a program with a `(Vec any)` BESIDE another `vec-of` emits one `-Wincompatible-pointer-types` warning from a `vec-new` spec that dedups across element types (either alone is clean; so are two concrete types). Measured cosmetic -- the two monomorphs have byte-identical layout -- but a warning in a user build all the same |
