...
| [any-carrier-straddle-is-bridged-per-consumer](any-carrier-straddle-is-bridged-per-consumer.md) | medium | A `(Vec any)` element is stored boxed, so the generic accessor hands it back as the slot's carrier WORD while its static type is `any` -- and every consumer wanting a real `tur_tagged_t` is a hard cc error until taught to bridge (`elab_coerce_to_any` cannot help: the type is ALREADY `any`, so there is nothing to widen). Three positions fixed one at a time over two days, each found by writing an ordinary Saffron program and watching it fail to compile: the `any` readers, an argument to an `any` parameter, and the dynamic-node operands. No forcing function enumerates the rest, unlike the three dynamic nodes where -Werror=switch did. Fix is to normalise once at PRODUCTION; the reverse CONCRETE->CARRIER crossing already exists for consumers that want the carrier. Never a wrong answer; the interpreter is unaffected |
