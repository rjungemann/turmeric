...
| [union-tagged-union-c-emission](union-tagged-union-c-emission.md) | low | per-member C union emission still deferred. FOUR defects hid behind it, three fixed: the unowned box at argument position, the aggregate match-arm compile error, and float truncation. **A fourth was found 2026-09-05 inside this entry's own "still open" note**, which said a union bound to a local or returned "still leaks its box" -- it did not leak, it did not COMPILE. `EX_UNION_INJECT` was inserted at call arguments and NOWHERE else, so `(let [u (:: (Wide ..) (Wide | int))] ..)` was "invalid initializer" and a `defn` declared `: (Wide | int)` was "incompatible types when returning". `elab_coerce_to_union` is the union twin of `elab_coerce_to_any`, now reached from the ascription and from return position (which needed a `return_union_type` capture -- `return_kind` alone is a bare TY_UNION with no member list). **Still open (1b): the heap box has no owner** -- 32,000 bytes in 1,000 blocks, measured, where the same program through `any` is clean. `let_binding_any_freeable` is a FOURTH rule keyed on TY_ANY, and adding the key is not enough: the drop channel emits `__tur_any_drop`, whose tag space (>= 1000) is disjoint from a union's member index, so it is a silent no-op. The report carries the shape of the fix and why a fall-through-only version would not even cover the natural repro |
