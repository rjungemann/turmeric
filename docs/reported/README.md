...
| [separator-fold-collides-emitted-c-names](separator-fold-collides-emitted-c-names.md) | medium | every non-alphanumeric character folds to `_` and emitted names join their parts with `_`/`__`, so the joiner is inside the folded alphabet and the split is ambiguous: ADT `a-b` ctor `c` and ADT `a` ctor `b-c` both spell `ctor_a_b_c`, and a user ADT named `Foo__int` is the `(Foo int)` monomorph's type name. Those arms are loud cc errors. The arm that sets the severity is SILENT: the bare-name ctor alias tests uniqueness on the RAW name but guards on the MANGLED one, so `b-c` and `b_c` both qualify, the second `#define` is dropped by its own `#ifndef`, and inline C calling `ctor_b_c` reaches the other ADT's constructor with no diagnostic at any layer (verified: asked for tag 0, got 1). Direction 1 closes the silent arm on its own for a one-line key change |
