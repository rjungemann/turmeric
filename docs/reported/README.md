...
| [vec-any-interp-keeps-one-element-tag](vec-any-interp-keeps-one-element-tag.md) | medium-high | A WRONG ANSWER, not a gap: under `--interpret` every element of a heterogeneous Vec reports the type of the LAST one pushed, and a cast to an element's real type panics naming a type the program never asked for. `native_vec_push` stores payload words in a raw int64 buffer and records ONE tag per vector -- its own comment calls it "the homogeneous element tag" -- and `vec-get` re-tags every element with it. The assumption held while the type system enforced homogeneity; `(Vec any)` is the first element type for which it does not, and `any` is exactly the type whose content IS the per-value tag. Compiled answers correctly (S6). `tests/fixtures/vec-any-element-roundtrip` carries requires.compiled so the divergence is recorded rather than hidden. Check Map and Set before S6's `#map{}`/`#set{}` work, not after |
