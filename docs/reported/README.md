...
| --- | --- | --- |
| [sr2-carrier-seam-rotted](sr2-carrier-seam-rotted.md) | medium | `TUR_SR2_APP_SUM_BYVALUE=0`, the bisection hatch left behind when `parametric-sum-byvalue` graduated, no longer produces correct programs. Three defects, two fixed. **FIXED 2026-09-04**: (1) the hatch aborted the compiler unless `TUR_OPTION_NICHE=0` moved with it -- `sr3_option_niche` now follows its substrate down; (2) `sum-passthrough-param-not-dropped` was an "invalid initializer" -- the match binder read an Option/Result pointer-box slot inline, fixed at both binder sites. Still open: (3) `option-niche-vec-closure-cmp`, a silent wrong answer -- no ABI specialization is minted on the carrier, so `Eq[Option]` keeps the `int` representative and compares String payloads by pointer; root-caused, fix is in the spec-minting trigger. Default path unaffected throughout. Found by restoring `tests/run-sr2-seam.sh`, retired at graduation |
| [defstruct-struct-path-is-dead](defstruct-struct-path-is-dead.md) | low | `defstruct_lowers_to_adt` has been widened slice by slice until it rejects nothing, so the `else` in `elab_defstruct` -- and the legacy StructDef elaboration behind it -- is unreachable. `elab_toplevel.c` already removed its counterpart branch (structdef-retirement DS-C), so the two files disagree. Measured: zero struct-path hits across 2781 fixtures, and `:heap` / `:linear` / parametric / applied-type / `exists` / fn / nested-aggregate all lower. Stale comments in both places corrected; the branch and its machinery left for the structdef-retirement track |
...
| [union-tagged-union-c-emission](union-tagged-union-c-emission.md) | low | per-member C union emission still deferred; the three defects it hid (unowned box, aggregate-arm compile error, float truncation) are fixed |
...
| [typed-comparator-over-hamt-box-compares-box-addresses](typed-comparator-over-hamt-box-compares-box-addresses.md) | medium | the residue of the row below, found by probing the scope claim that row asserted rather than trusting it. The `*-eq?` family splits into "walks its own payload slots" (word) and "iterates a HAMT" (box), not "Vec versus the rest": `map-eq?` / `set-eq-cmp?` hand over a box, so their TYPED comparator reinterprets it and compares box ADDRESSES -- a value is unequal to itself. That is the spelling TUR-E0715 and the `vec-eq?` docstring both now recommend, so the two families advise opposite things. Plus two smaller residues: the `pair-eq?` MACRO applies its comparator directly to a field read (no callee to hang a contract on), and `option-eq?` rejects a hand-written lambda with `expected (fn [int int] : bool), got (fn [int int] : bool)` |
| ~~erased-closure-param-over-niche-vec-slot-reads-box~~ | -- | **Resolved 2026-09-04**, and its own scope claim CORRECTED the same day (the row above): the synthesized `(eq? a b)` comparator was marked for `Vec` alone, so `(eq? (some (some "aa")) (some (some "bb")))` -- the DEFAULT equality operator on a nested niche option -- answered true. `container_helper_passes_slot_words` replaces that name test, and `list-eq?` / `result-eq?` join `vec-eq?` at the user-written site. Both fix directions, each where the other could not reach. A comparator written AT the `vec-eq?` call is minted for that argument and reachable from nowhere else, so it now carries the same `__cmp_slot_` mark the synthesized one does (capturing as an `EX_CLOSURE`, captureless through its lifted `__fn_N` -- `is_lifted_lambda`); the mark is the emitted NAME because the value table is program-scoped, and a flag-plus-`emit_slot_word_mark` would have made every unrelated local named `a` a slot word. A NAMED comparator cannot be marked -- another caller may hand it real boxes -- so erased params there are refused with TUR-E0715, the read-side twin of E0714; it fires on zero of 2784 fixtures. The filed repro understated it: with DIFFERENT payloads the broken path printed `eq`, a wrong answer rather than a right one reached wrongly. Archived to [docs/archive](../archive/erased-closure-param-over-niche-vec-slot-reads-box.md) |
