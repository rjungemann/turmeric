...
| [generic-wrapper-tail-forwarding-a-return-dispatch-method](generic-wrapper-tail-forwarding-a-return-dispatch-method.md) | low | **Narrowed 2026-09-13/14; the cc error it was opened for is FIXED.** A generic wrapper whose body is a direct tail call to a return-type-directed class method (the `encode-string` / `decode-list` shape) failed codegen -- the spec declared a by-value `Result` while the instance returned the int64 carrier -- and now routes through `emit_carrier_bridge` at the return. The follow-on payload leak is fixed too (the wrapper's binding is flagged `returns_fresh_sum_box` from the same minting-shape contract the inline-C rule uses). Nothing measurable remains; kept open only until a second consumer confirms the shape. Pinned by `tests/fixtures/generic-wrapper-tail-forwards-return-dispatch` |
...
| Report | Severity | One line |
| --- | --- | --- |
| [perform-does-not-typecheck-its-arguments](perform-does-not-typecheck-its-arguments.md) | **medium** (was high) | **NARROWED 2026-09-15: the memory-safety half is closed.** `elab_perform` now rejects a LITERAL scalar (int/bool/float) reaching a POINTER-shaped parameter (`cstr`, `ptr<T>`, ref/rc/weak, a borrow) with the ordinary `TUR-E0001`. One notch narrower than the filing proposed, and the narrowing is measured: it says that shape "admits no legitimate program", and it admits one -- an UNANNOTATED lambda parameter defaults to `int` and is refined later, so `(fn [msg] (perform (Log msg)))` against a `cstr` parameter is correct and read as int here; the broader rule broke 7 fixtures on it, which are now this check's controls. A literal's type is never provisional, so the rule has no false positives. Runs after both `any` seams, so an unboxed `any` is judged on what it became. Pinned by `errors/perform-arg-scalar-into-pointer-param`. STILL OPEN: the general check, and with it the same mismatch arriving through a VARIABLE rather than a literal, a cstr-into-`:int` argument, float/int confusion, a wrong aggregate def, and **arity** (`n_args` is never compared against `n_params`, so extra arguments are silently dropped). Original row: `(perform (Log 42))` against `Log [msg : cstr]` compiles clean in plain TYPED Turmeric and segfaults -- `elab_perform` stores each argument without ever reading the declared parameter types the handler goes on to use. Not a one-liner: a general check has to agree with the ~300 lines of `arg_ok` logic at `elab_call.c:6195`, which wants factoring out first. The `any` seam fixed above is the corner of this hole, not the hole |
