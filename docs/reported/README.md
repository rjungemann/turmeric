...
| [typeclass-method-resolution-ignores-the-class](typeclass-method-resolution-ignores-the-class.md) | medium | **Narrowed 2026-09-11: three of four items closed.** Symptom B (unconstrained generic passes `tur check`, then fails in cc) is FIXED and verified -- a clean check-time TUR-E0015 naming the class, the tyvar and the constraint to add. The "trap" (a `^borrow` caret in a `definstance` method impl consumed a param slot, emitting a two-parameter C function that cc rejected at every call site while `tur check` exited 0) is FIXED -- the `defclass` parser had skipped carets since ECS E2d-P6 and the instance-impl parser did not; both loops now carry the guard, pinned by `definstance-borrow-caret-in-impl`. The docs correction owed to `ecs-component-set-bounds-plan` is DONE. **Still open: symptom A** -- a `definstance` below its use still does not resolve. The diagnostic is accurate and names the workaround (move it above), so this is a papercut, not a mystery; making registration order-independent is a scoped project (recursive registration walk + position-derived Pass 2 state + a signature-only resolution path), not a patch |
