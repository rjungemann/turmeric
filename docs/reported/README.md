...
| [direct-call-does-not-inherit-caller-tyvar-binding](direct-call-does-not-inherit-caller-tyvar-binding.md) | medium | The compiled path answers one question two ways: a constrained generic whose tyvar appears in NO parameter inherits the caller's binding when PASSED as a function value, but not when CALLED -- `viadirect` gets no specialization while `viafold` gets one per instantiation. **The 'is it well-formed' question is SETTLED** (2026-09-12): alpha-renaming the callee's `[V]` to `[W]` used to change the answer on BOTH back ends, so the inheritance was name CAPTURE, not resolution -- `crdt/ormap` was one rename from silently merging with the wrong join (verified by doing it). Both paths are class-keyed now (`emit_translate_bindings_by_class`; `frame_lookup_dict` in eval.c), with alpha-renamed fixture rows on both back ends. **Still open:** the direct-call half, which should resolve through the caller's dictionary for the same class the way the fn-value half now does. |
