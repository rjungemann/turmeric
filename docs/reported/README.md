...
| [direct-call-does-not-inherit-caller-tyvar-binding](direct-call-does-not-inherit-caller-tyvar-binding.md) | medium | The compiled path answers one question two ways: a constrained generic whose tyvar appears in NO parameter inherits the caller's binding when PASSED as a function value, but not when CALLED -- `viadirect` gets no specialization at all while `viafold` gets one per instantiation, so the direct form silently answers with the first instance. The call site can derive no binding (nothing pins `V`), so the relay probe does not fire; the fn-value path instead hands the ENCLOSING spec's bindings to the clone. **Whether the call is even well-formed is the open question** -- the two `V`s are independent variables sharing a spelling, so a Haskell-like reading calls it ambiguous -- but the two paths must not disagree. Inheriting is the more useful reading and is what makes the `crdt/ormap` idiom work. Fixture owed once settled. |
