...
| [global-mut-cell-of-byvalue-adt-or-fn-emits-bad-c](global-mut-cell-of-byvalue-adt-or-fn-emits-bad-c.md) | low-medium | Filed 2026-09-19. `tur check` passes but cc rejects the emitted C for (1) `(set! g t)` where `t` is a match binder of a by-value recursive ADT and `g` a `^mut` global (pointer assigned to aggregate), and (2) a `^mut` global fn cell that is `set!` and then called (direct C call through the carrier). Both are the natural way to write a retaining callee, the negative control the ownership inference needs |
