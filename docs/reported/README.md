...
| [option-rc-payload-constructible-only-from-inline-c](option-rc-payload-constructible-only-from-inline-c.md) | medium | `(Option rc<A>)` can be RETURNED, built in inline C, and matched -- but `(some x)` and the generic `unwrap` are both rejected with "cannot store an owning value (rc) in a collection", with no collection in the program. The check fires on a generic callee's tyvar-parameter erasure (`elab_call.c:5613` -> `:1066`), and its stated rationale is stale: a concrete monomorph's payload slot is a typed `RcControlBlock *`, not an int64 carrier (SR2a). Still correct for the ERASED path, so the fix must tell the two apart. Costs stdlib a hand-rolled `weak/unwrap` and pushes authors toward the inline-C form. `Result` affected identically |
