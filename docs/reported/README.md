...
| [map-of-any-is-broken-on-both-back-ends](map-of-any-is-broken-on-both-back-ends.md) | medium-high | Filed to SETTLE a question rather than react to a bug: the Vec element-tag fix ended by saying Map and Set should be checked BEFORE S6's `#map{}`/`#set{}` work, because the Vec case was a silent wrong answer that only surfaced once a heterogeneous container became constructible. Measured: `(Map int any)` reports `int` for every value under --interpret and emits uncompilable C compiled; `(Set any)` refuses cleanly at elaboration on both. Map is NOT Vec's problem again -- it has no tag side table at all, `native_map_get_eq` returns `turi_int` of the raw carrier word, so there is no mechanism to extend. A homogeneous `(Map int float)` is fine: the tag comes from the STATIC element type, which `any` does not have. Best direction is a boxed TuriValue as the HAMT value, matching what the compiled path already does for a `(Vec any)` element, so the back ends agree by construction; direction 3 (refuse it like Set does) is worth taking immediately if that is not imminent -- a wrong answer should not survive contact with S6 |
| [vec-any-monomorph-is-half-plumbed](vec-any-monomorph-is-half-plumbed.md) | medium | With the ICE fixed a `(Vec any)` builds, stores and frees -- and is write-only: `vec-get` reports `int`, so an element cannot be read back with its tag (identical on both back ends, so elaboration; it is S6's actual subject, since the same question governs `#map{}`/`#set{}`/cons lists). Second half, **corrected twice from "cosmetic"**: a `vec-of` producing a `(Vec any)` emits `-Wincompatible-pointer-types`, which run.sh's emitted-C ratchet FAILS -- that gate was sweep-verified at zero across 2563 fixtures, so a hit is a regression signal and a hard error under -Werror. Both wrong calls came from not asking the suite: first from comparing the two monomorphs' layouts, then from manual -O1 probes when run.sh builds at -O2. Only the explicit `(:: (vec-new) (Vec any))` route is shippable today. Cause narrowed: the zero-arg `vec-new` inside `vec-empty-like__`'s `any` clone resolves to the `int` clone, and emit_call_name short-circuits a zero-arg call before the spec-matching loop |
