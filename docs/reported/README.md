...
| [pair-eq-macro-applies-comparator-directly](pair-eq-macro-applies-comparator-directly.md) | low-medium | the last residue of the row below. `pair-eq?` is a MACRO that splices the comparator into call position against a `.fst` field read, so unlike every other `*-eq?` there is no callee whose parameter contract could say which convention the value arrives in -- and both mechanisms that resolve this elsewhere key on exactly such a call. An erased comparator over a niche Pair element prints "eq" for different payloads; `(eq? p1 p2)` and the typed spelling are both correct. The legacy `pair-eq-carrier?` helper WOULD fit the existing mechanism (one line) but does not fix the macro, so it was deliberately left out rather than imply coverage |
