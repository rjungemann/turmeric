...
| [codegen-carrier-straddle-in-lifted-thunk-sink](codegen-carrier-straddle-in-lifted-thunk-sink.md) | medium | **Filed 2026-09-07, found by `tests/regions-fuzz-src.py`** (and nothing to do with regions -- it reproduces with `TUR_REGIONS=0`, with no region form, on `origin/main`). A lifted closure thunk whose body is a `do` with a discarded first item and a tail call returning the int64 carrier for a `:heap` ADT gets a result sink declared with the CONCRETE POINTER type; the sink assignment is unbridged, so the emitted C does `tur_adt_TL * __t = <int64_t>`. The return out of the same function IS bridged -- only the sink is not. Hard `cc` error on clang / GCC >= 14 (macOS CI red), a warning on older GCC; `run.sh`'s own pointer/integer ratchet greps for exactly that warning and stays quiet only because no fixture has the shape. Fix direction: bridge at the stackless result-sink write (`emit_fns.c`, `sink->dest` / `gs->vars[].cname`) with `emit_carrier_bridge(CK_CARRIER, CK_CONCRETE)`, or declare the sink as the carrier; wants its own change plus a snapshot regen and the repro pinned as a fixture |
