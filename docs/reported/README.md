...
| [definstance-not-dispatchable-across-modules](definstance-not-dispatchable-across-modules.md) | medium | A `definstance` in module A is not dispatchable from module B: `tur check` is clean and cc fails with `call to undeclared function '__inst_<Class>_<method>_<Type>'`. Elaboration resolved the right mangled name -- the instance defined in the SAME module is declared in that TU and the imported one is not -- so it looks like an emission gap, not a scoping decision. Adding the type to `:refer` does not help. Workaround is an exported forwarder, which costs the abstraction: a downstream module cannot be generic over the class for a type whose instance lives elsewhere. Needs a two-module fixture; the single-file path has no boundary to cross |
