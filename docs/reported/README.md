...
| [fn-cell-set-with-capturing-closure-segfaults](fn-cell-set-with-capturing-closure-segfaults.md) | medium | Filed 2026-09-19. A `^mut` fn cell (local or global) initialised with a non-capturing lambda and re-pointed at a CAPTURING closure segfaults when called: the cell is spelled thin, the stored value is a fat box, and nothing records the change |
