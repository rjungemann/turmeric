...
| [fn-typed-param-forwarded-to-a-later-defn-miscasts](fn-typed-param-forwarded-to-a-later-defn-miscasts.md) | medium-high | **BUILD FAILURE whose trigger is source ORDER.** Forwarding an `(fn ...)`-typed parameter to a defn defined LATER in the file emits the erasing carrier cast `(void *)(intptr_t)(g)` onto a `tur_poly_fn_t`, which is a struct: `operand of type 'tur_poly_fn_t' where arithmetic or pointer type is required`. Moving the callee ABOVE the caller compiles the identical program. The forward declaration already carries the right parameter type, so the call-site emitter is reading a table populated as definitions are emitted rather than the declaration pass. Not about `^borrow`, structs, `let`, or recursion -- each was ruled out separately. Worked around in `crdt/ormap` by defining helpers first. Fixtures owed. |
...
| [phantom-type-param-does-not-drive-monomorphization](phantom-type-param-does-not-drive-monomorphization.md) | high | **SILENT WRONG ANSWER.** A constrained generic whose type variable appears only in a PHANTOM position (named in a `defstruct` head, used in no field) gets **no `__spec__` specialization at all** -- it is emitted once at the carrier and the constraint resolves to the representative instance. `(Ph int)` and `(Ph Sum)` both lower to `int64_t`, so the specialization key, which is derived from lowered argument types, cannot tell them apart. Both instances are emitted; only one is ever called. Ruled out by measurement: type var direct, through a struct with a TYPED field, laundered through an int carrier, and `defopaque` over the same carrier are all CORRECT -- phantomness alone is the trigger, and unlike the turi report this needs no nesting and is compiled-path. **Blocks crdt-spice-plan 2.3's `ORMap K V`**, whose entries live in a HAMT of int carriers so `V` is necessarily phantom; C3 ships an explicit value-join parameter instead. Fixtures owed. |
