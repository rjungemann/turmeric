...
| Report | Severity | One line |
| --- | --- | --- |
| ~~try-turmeric-browser-suites-green-while-failing~~ | -- | **Resolved 2026-09-02**: the Playwright report is uploaded whenever the job is not cancelled, the desktop/mobile suites write JUnit that `collect-playwright-timings.py` turns into honest `web_desktop` / `web_mobile` rows for `/ci`, the job summary and a warning annotation name a failed non-blocking suite, and the three test-side failures (unscoped minimap selectors, a non-configurable `location.reload` stub) are fixed; the mobile WebKit reload failure is its own report. Archived to [docs/archive](../archive/try-turmeric-browser-suites-green-while-failing.md) |
...
| Report | Severity | One line |
|---|---|---|
| [try-turmeric-mobile-reload-fails-wasm-init](try-turmeric-mobile-reload-fails-wasm-init.md) | medium if real | the mobile (WebKit) Playwright project shows "Failed to load WASM" after `page.reload()` in two specs; unattributed -- may be the sw-controlled-reload shape, may be a real iOS Safari bug |
