...
## Found auditing body-taking control forms (filed 2026-09-21)

| Report | Severity | One line |
| --- | --- | --- |
| [single-body-control-forms-require-an-explicit-do](single-body-control-forms-require-an-explicit-do.md) | medium (one **high** sub-case) | `when`, `unless` and a dozen other body-taking forms accept **exactly one** form, so every multi-statement body pays an explicit `(do ...)`. `(when c a b)` is a hard error whose primary location is `stdlib/macros.tur:55`, with no `TUR-Exxxx` code. **`(defer a b)` silently drops `b`** on both back ends -- `elab_defer` elaborates `items[1]` and never reads 2..n (`elab_forms.c:3947,3956`); that is the high sub-case. The stdlib is split down the middle: **12 `& body` macros against 14 single-`body` ones**, `with-resource` and `when` eleven lines apart in the same file. The rest of the toolchain already assumes variadic -- `fmt_when` lays out an N-form body (`fmt.c:1278`) and `docstrings.c:238` reads `(when cond body ...)`. Measured tax: **487 single-body call sites in-tree, 28 of them wrapping in `(do ...)`** (13% of `when`, 14% of `serial-reset`). Identical under `#lang saffron`, which shares the prelude. One-line `& body` fix for `when`/`unless` was trialed: **3077 passed, 12 failed, all 12 pre-existing `no runnable input` on stale artifact dirs**; no fixture changed behavior, no snapshot moved; reverted. `cond`/`case`/`match`/`handle` are explicitly NOT candidates -- their flat pair shape makes a body block inexpressible |
