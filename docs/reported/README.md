...
| [float32-block-temp-widens-to-double](float32-block-temp-widens-to-double.md) | low | A float literal closing a MULTI-expression `: float32` body gets a `double` temp and narrows on return; a single-expression body is fine. Values are right today (exact literals), but a non-representable one would round twice. Visible only under GCC, whose `-Wfloat-conversion` covers double->float where clang splits it out -- which is how the Linux leg found it. `tests/run.sh`'s ratchet excludes float-destination warnings so this does not fail the suite; **narrow that exclusion when this lands** |
