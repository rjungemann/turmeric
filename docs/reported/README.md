...
| [sr4-byvalue-recursive-sum-walk-copies-per-link](sr4-byvalue-recursive-sum-walk-copies-per-link.md) | medium | walking a self-recursive by-value sum copies **120 bytes per link**; `logic.tur`'s `Subst` chain is several times slower than `TUR_SR4_RECURSIVE_CARRIER=1`, growing with chain length. **Fix (1) landed 2026-09-04 and REFRAMES the report**: removing copy (c) -- 48 of the 120 bytes, 40% of the volume -- moved the benchmark ~5%, and the by-value/carrier ratio still reads 8.83x at n=512 afterwards. So the copies are real but NOT the dominant term, and the report's plan ("remove the copies and the cost goes with most of them") is not supported by its own first data point. (2) and (3) remain, but the report now says to PROFILE first -- cache footprint, construction vs traversal, and dependent-load latency are all unaccounted for, and only one of them is a copy |
...
| [capturing-thunk-returning-heap-field-record-garbles-int](capturing-thunk-returning-heap-field-record-garbles-int.md) | medium-high | a CAPTURING fat thunk returning a by-value record that holds a `:heap` ADT field reads the record's INT field back as nondeterministic pointer-sized garbage on the COMPILED path; the interpreter is correct. Needs both variables: a non-capturing thunk is fine, and an all-scalar (or `:float` / `:cstr`) record through the same capturing thunk is fine. The generic-bracket shape is exactly `bt-scope` / `with-region`, so returning a heap handle in a record out of either bracket is wrong today -- independent of the region, which correctly retires such a result. Found 2026-09-05 writing `region-scope-shapes`; no region in the repro. Root cause not established; lead is the fat-closure by-value-aggregate return path gated on a typed-pointer field |
...
| [windows-spice-fetch-shell-quoting](windows-spice-fetch-shell-quoting.md) | high (Windows) | `tur fetch` could not fetch anything: POSIX `'...'` quoting interpolated into a string cmd.exe runs, which passes `'` through literally (`could not create leading directories of ''./spices/demo''`). Fixed in `pkg.c`; the same defect in `install.c` is still open |
