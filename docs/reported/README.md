...
| [map-of-any-is-broken-on-both-back-ends](map-of-any-is-broken-on-both-back-ends.md) | medium-high | Filed to SETTLE a question rather than react to a bug, before S6's `#map{}` work. **Compiled half FIXED 2026-09-08**: the store side was already right (the HAMT assoc boxes the value, since repr_of answers BOXED_AGG at CONTAINER_ELEM), and only the read was missing its deref -- a `tur_tagged_t` temp assigned the raw slot word, which `bridge_control_result_int_ptr` bailed on rather than handling as the third of its straddle pairs. **Interpreter half open and it is the wrong-answer one**: `(Map int any)` reports `int` for every value, no diagnostic. Map has no tag side table at all (`native_map_get_eq` returns `turi_int` of the raw word), and the Vec analogue is ruled out -- that table keys on an element INDEX the trie does not expose. Store a boxed TuriValue instead, matching what the compiled path does. `(Set any)` refuses cleanly at elaboration on both, which is the good failure |
