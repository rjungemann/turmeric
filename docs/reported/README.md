...
| [narrow-closure-result-read-through-int64-carrier](narrow-closure-result-read-through-int64-carrier.md) | high | **Filed 2026-09-25.** A `bool`-returning closure called through the erased int64 carrier reads the whole return register, of which only the low byte is defined, so a false result can print as true on the compiled path (`--interpret` is correct). Reproduces through `Result`'s existing `fmap` on a mixed-type receiver; `ap` on `Result` inherits it. Fix is on the callee side: widen narrow integer results at every closure entry point an erased caller can reach |
| [defdata-ctor-fn-field-passes-pointer-as-int](defdata-ctor-fn-field-passes-pointer-as-int.md) | low | **Filed 2026-09-25.** A user `defdata` constructor declares a function-typed field as `int64_t` while the call site passes a capturing closure's handle as `void *`, a `-Wint-conversion` warning that `tests/run.sh`'s representation check rejects. The program runs correctly |
| [hkt-generic-forwarded-bind-continuation-segfaults](hkt-generic-forwarded-bind-continuation-segfaults.md) | high | **Filed 2026-09-25.** A `[^Monad M]` generic that passes a continuation PARAMETER to `bind` segfaults compiled at `Option` and `Result`; `--interpret` is correct, and a continuation written as a lambda in the body works. Reproduces before the SC8b work with every constraint spelled out |
| [hkt-dict-generic-byvalue-result-to-typed-param](hkt-dict-generic-byvalue-result-to-typed-param.md) | medium | **Filed 2026-09-25.** A dictionary-passing HKT generic's result over a user by-value type (`(Tally int)`) is a C type error when passed to a typed parameter, and `fmap`'s result fed to `foldl` in such a generic is an invalid initializer; a `match` on the call works. No wrong answers |
| [generic-category-base-passes-carrier-to-arrow-instance](generic-category-base-passes-carrier-to-arrow-instance.md) | low | **Filed 2026-09-25.** A generic over `Category` (or `Arrow`) called at the function arrow emits `-Wint-conversion`: its unspecialized base passes `int64_t` carriers to the `(->)` instance's `void *` parameters. The program is correct; the harness's representation check rejects the C |
| [hkt-generic-none-to-typed-param-segfaults](hkt-generic-none-to-typed-param-segfaults.md) | high | **Filed 2026-09-25.** A constrained HKT generic that returns `none` at `Option` segfaults compiled when its result is passed to a typed `(Option int)` parameter; `some`, an inline `match`, and `--interpret` are all correct. Reproduces before the SC8b work with every constraint spelled out |
| [hkt-generic-nested-bind-result-type](hkt-generic-nested-bind-result-type.md) | medium | **Filed 2026-09-25.** A two-binding `do-m` in a constrained generic is a C type error (the inner `bind`'s by-value result assigned to a carrier temp) at `Option` and `Result`; one binding works, and `--interpret` is correct |
