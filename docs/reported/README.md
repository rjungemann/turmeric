...
| [interp-collection-handles-report-as-int](interp-collection-handles-report-as-int.md) | medium-high | The interpreter represents a Vec/Map as a raw `TURI_INT` holding a pointer, so `(defn f [] : any (vec-of 1 2 3))` gives `type-of` "Vec" compiled and "int" interpreted -- and `is?` is wrong in BOTH directions: a false negative on `(Vec int)`, a false positive on `int`, so a type-case with an `int` arm and a `Vec` arm takes different arms on the two back ends. A representation gap, not a missing link (there is no struct to hang a type on), so filed separately from the native-ctor row above that the same sweep resolved |
| [union-to-any-widen-emits-uncompilable-c](union-to-any-widen-emits-uncompilable-c.md) | medium | Widening a union-typed value to `any` emits `TUR_TAG(38, (int64_t)(intptr_t)(x))` -- the scalar arm -- on a value that is already a `tur_tagged_t`, so cc rejects it with "aggregate value used where an integer was expected" in argument, return and local position alike. The interpreter runs the same program and answers correctly, so the semantics are settled and only codegen is missing. The real question is which tag the `any` should carry: a union's tag is a MEMBER INDEX, an `any` box's is a TypeKind or type id. Found fixing the guide, whose gradual-typing example was exactly this shape |
| [partial-application-widened-to-any-is-a-ptr](partial-application-widened-to-any-is-a-ptr.md) | medium | An under-saturated call's static type is `ptr<void>`, not a `TY_FN`, so `(defn mkp [] : any (add 1))` widens a POINTER: `type-of` answers "ptr" compiled and "fn" interpreted, and no `is?` target matches -- a curried closure goes into an `any` and cannot come back out. Currying is the arity guide's recommended way to bake in defaults, so this is a load-bearing hole for the Saffron layer, not a curiosity. Found probing the fn-id fix above; independent of it |
