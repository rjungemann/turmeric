...
| [erased-generic-field-read-overruns-subword-monomorph-box](erased-generic-field-read-overruns-subword-monomorph-box.md) | medium | a by-value monomorph with a sub-word field (`(Identity bool)`, 1 byte) is heap-boxed at the aggregate return spill with `sizeof(<monomorph>)`, and the ERASED generic reader (`run-id`'s `(.wrapped i)`) then reads it through the erased layout, 8 bytes wide: ASan heap-buffer-overflow, right answer by little-endian luck. Pre-existing on `main`; found by the first corpus-wide ASan sweep. Fix: box to the erased width at the spill |
