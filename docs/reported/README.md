...
| [return-dispatched-sum-mint-in-constrained-instance-miscompiles](return-dispatched-sum-mint-in-constrained-instance-miscompiles.md) | medium | a class method returning the bare class var whose Option instance mints its element by re-dispatch fails at `cc` three ways (a by-value spec result deref'd as the carrier at the ctor argument; the dictionary slot's base clone declared with a by-value result over a carrier body; a concrete applied instance head reading `Box` as a tyvar). `tur check` is silent. Found writing the round-4 fixture of the report above; the reader-mediated spelling compiles |
