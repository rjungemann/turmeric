...
| ~~[turi-fiber-recv-timeout-ignores-its-deadline](../archive/turi-fiber-recv-timeout-ignores-its-deadline.md)~~ | medium | **RESOLVED 2026-09-17** (archived): the fiber arm now arms one scheduler timer for the remaining wait and registers the fiber as its waker alongside `ch->recv_waiter`, so either the peer's deposit or the alarm resumes it and the loop re-checks the clock; the alarm is settled on every exit so it cannot fire at a fiber that is no longer parked. A second hazard the report did not name was fixed with it: `session_wake` left its waiter marked `SUSPENDED` while it sat on the ready queue, which with a second wake source is a double-enqueue that TRUNCATES the ready queue (`turi_sched_enqueue` clears `sched_next`) -- it now marks `READY` first, as `flush_wakers` already did. Pinned by `tests/fixtures/session-timeout-fiber-turi`, which asserts ORDER (`fiber-timeout` before the peer's 800ms send) rather than just `timeout`, since a `timeout`-only assertion cannot tell an observed deadline from a peer that never sends. This clears the step-4 prerequisite the multi-party-timeout row below names; its steps 1-2 are untouched. Original row: A `recv-timeout` inside an `async` fiber under `--interpret` waits indefinitely and takes the **Left (success)** branch on a value that arrives long past the deadline; compiled takes **Right (timeout)**. Silent wrong branch, no diagnostic. The same op in the **main** context is correct, which is why no fixture catches it |
| ~~[awaited-sleep-async-in-a-fiber-drops-the-rest-of-the-body](../archive/awaited-sleep-async-in-a-fiber-drops-the-rest-of-the-body.md)~~ | medium | **RESOLVED 2026-09-17** (archived): `native_sleep_async`'s fiber arm violated its own `-> Future` contract -- it blocked on the timer correctly and then returned `turi_nil()`, so `EX_AWAIT` failed its tag check, and that error propagated out of the body and REJECTED the fiber's own future. Nothing unwound and nothing was reclaimed early: the report's suspected mechanism was wrong, and the rejection was visible all along one level up, as `(println (await t))` shows. It now returns the (already-resolved) future, so both spellings work. The generalisation the report suspected is real and was fixed with it: `native_read_async` and `native_write_async` have the identical shape. `session-timeout-expired-turi` did NOT need its `await` dropped to stop being vacuous -- with the fix its peer does reach the `send`, probed directly. Pinned by `tests/fixtures/await-sleep-async-in-fiber-turi`, which carries the bare-`sleep-async` control in the same program. Original row: Under `--interpret`, an `async` fiber that sleeps via `(await (sleep-async n))` **never runs anything after the sleep**; the future resolves anyway, `(await t)` returns, the program exits 0 |
