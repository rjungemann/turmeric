...
| [interp-inline-c-opaque-segv-in-any-reflection](interp-inline-c-opaque-segv-in-any-reflection.md) | **high** (crash) | `type-of` on an `any` holding a `defopaque` value returned from an INLINE-C body segfaults under `--interpret`: the value arrives TURI_STRUCT-tagged with `as_struct` set to the raw integer the body returned (0x7), and `turi_any_named_type` checks only the tag and non-NULL before dereferencing it. The ascription form `(:: 7 Route)` does not crash, which localises it to the inline-C return path. Pre-existing and invisible because `run-turi.sh` PASS-skips every inline-C fixture (~767 of them) |
...
| [set-of-element-type-is-not-checked](set-of-element-type-is-not-checked.md) | medium | `(set-of 1 "two")` type-checks as `(Set int)` and passes to a `(Set int)` parameter while holding a cstr pointer -- plain Turmeric, no `any`, both back ends. `vec-of` routes elements through `tur-vec-homog__` and `hamt-of` through `tur-map-homog__`; `set-of` has no counterpart, so each `set-add1` resolves `hash`/`mk-box`/`mk-cmp` at its OWN element's type. That is why the runtime is right -- content-keyed dedup, membership by value -- and only the exported type is a lie. Fix is the missing `tur-set-homog__` twin; admitting `any` elements is a separate feature that should not gate it |
| [map-literal-leaks-its-intermediate-maps](map-literal-leaks-its-intermediate-maps.md) | medium | `#map{:a 1 :b 2 :c 3}` leaks 1080 bytes in 11 allocations WITH `map-free` called -- plain Turmeric, no `any`. `hamt-of` chains `map-assoc` and each step mints a fresh `{void* hamt}` carrier box (a HAMT is persistent), so an N-entry literal mints N boxes and the author holds only the last. Bounded per literal, unbounded in a loop, and nothing the author can free. The interpreter is unaffected -- `set_wrap_tracked` registers every box on the env. Fix is to free the previous generation inside the literal's own lowering, where every intermediate is owned by construction |
| [any-carrier-straddle-is-bridged-per-consumer](any-carrier-straddle-is-bridged-per-consumer.md) | medium | A `(Vec any)` element is stored boxed, so the generic accessor hands it back as the slot's carrier WORD while its static type is `any` -- and every consumer wanting a real `tur_tagged_t` is a hard cc error until taught to bridge (`elab_coerce_to_any` cannot help: the type is ALREADY `any`, so there is nothing to widen). FIVE positions fixed one at a time over two days, each found by writing an ordinary Saffron program and watching it fail to compile: the `any` readers, an argument to an `any` parameter, the dynamic-node operands, a `let` binding declared `any`, and an `any` slot of a fat-closure dispatch (the last two both found by one `vec-map` in the Saffron prelude). No forcing function enumerates the rest, unlike the three dynamic nodes where -Werror=switch did. The fix is to normalise once at PRODUCTION -- but measured, `emit_value`'s call hoist declares its temp `__auto_type` ON PURPOSE, so the emitter does not know the C type at the one chokepoint where the bridge belongs; that also explains why the consumer bridges kept declining on the very values they exist for (a hoisted call temp was absent from the carrier-representation side table until the `ret_ct` arm was taught to record it). Never a wrong answer; the interpreter is unaffected |
| [any-narrowing-ascription-does-not-compile](any-narrowing-ascription-does-not-compile.md) | medium | Narrowing an `any` back to a concrete type with `::` is `error: aggregate value used where an integer was expected` on the compiled path (an `any` is the two-word `tur_tagged_t`, so there is no single word to bit-reinterpret), and under `--interpret` prints the RAW CARRIER WORD with no diagnostic -- `(:: a int)` on an `any` holding 7.1 answers `4619679907765970534`. Not a missing capability: `cast` narrows correctly on both back ends and panics on a tag mismatch, identically. Both back ends are doing what `::` means (an unchecked representation assertion); the defect is that `::` accepts an `any` operand at all. Fix is one TY_ANY arm rejecting it and naming `cast`. Found while measuring the Map report |
