...
| [saffron-any-return-defeats-the-frame-box-rule](saffron-any-return-defeats-the-frame-box-rule.md) | medium | Widening a by-value payload into an `any` PARAMETER mallocs a box nothing frees, once per call. `any-struct-box-leak-per-widen`'s first pass puts that copy in the caller's frame instead -- but only when the callee's RESULT is a non-pointer scalar, since an `any` result can carry the payload pointer back out. The gate is right; what changed is how often it is reached. Saffron's unannotated return IS `any` (D3), so `_result_safe` is false for essentially every Saffron function and a rule written for the rare case now never fires in the language where the widen is the common case. Measured at S5 with `tests/fixtures/saffron-dyn-field` (16 bytes, 1 allocation). The sibling `saffron-higher-order` leak is NOT this: a plain Turmeric recursive ADT leaks one box per cons cell too (72 bytes / 3 cells, measured), so that one is the existing no-drop-glue story wearing `any` boxes. Both carry `known-leak` so the gate stays readable. Three fix directions filed; the plan's own S5 note ("ownership is the real risk here, not the dispatch") proposed the largest of them |
| [vec-of-any-repr-decision-ice](vec-of-any-repr-decision-ice.md) | **high** (ICE) | `(vec-of (id 1))` where `id : any -> any` aborts the compiler: `a representation decision disagrees with repr_of at binding -- type=(type-app Vec any) want=heap-ptr got=carrier-i64`. Pre-existing and NOT Saffron-specific (reproduces in plain Turmeric with explicit annotations), but S2's `any` default makes it trivially reachable. The interpreter runs the same program fine, so only the compiled representation decision is in question. An `any` from an in-place ascription trips a DIFFERENT defect instead (`TUR-E0201 cannot copy unique value '__vw'` inside stdlib/vec.tur), so both routes to a `(Vec any)` are broken in different ways. Blocks S6 |
