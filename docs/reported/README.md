...
| [saffron-applied-class-var-result-takes-one-instances-type](saffron-applied-class-var-result-takes-one-instances-type.md) | medium | Filed 2026-09-26 (saffron-lang-plan S9). A one-parameter method declared `: (Option a)` and dispatched on an `any` receiver takes its result type from whichever instance the static resolver picked, so with two instances the other's result is typed -- and boxed -- as the first's: `(is? (w 4) (Option any))` is false compiled (true interpreted) and a cast of the Pt instance's result panics. The star-witness path is the fix, but its witness does not compile until an instance body sees its substituted declared result as the expected type. |
