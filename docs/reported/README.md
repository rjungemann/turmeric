...
## Emitter return paths (filed 2026-09-22)

Found landing proper-tail-calls T2. One report, and it is the thing that keeps
T2's gate narrow.

| Report | Severity | One line |
| --- | --- | --- |
| [emit-tail-return-path-lacks-carrier-bridges](emit-tail-return-path-lacks-carrier-bridges.md) | medium | Two hand-maintained copies of "how does a body's value reach the `return`" -- `emit_fn_def`'s final `else` and `emit_tail`'s default arm -- and `emit_tail`'s has a strict subset of the other's carrier/dict/straddle bridges. Which one a body gets is decided by whether its tail spine happens to hold a self tail call, which is unrelated to its result shape. Unreachable today only because T2's marking refuses any non-self tail call whose callee's C return type is not exactly the enclosing function's; widen that gate by even the difference between a generic name and its `__spec__` name and 24 fixtures fail with hard `cc` errors. Fix: factor the ladder into one function both paths call, the treatment `inline_c_returns_byvalue_adt` already got. Also the unblock for T2's unreached half |
