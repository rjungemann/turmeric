...
| [hkt-carrier-result-loses-payload-types](hkt-carrier-result-loses-payload-types.md) | high | On the TYPED path, `fmap` on a heterogeneous `(Result int cstr)` or `(Either int cstr)` prints a cstr payload as its ADDRESS -- a silent wrong answer, no diagnostic. The carrier dispatch's result type never grounds, so a `match` on it types every binder as the int carrier. Taken as a baseline measurement while fixing the Saffron pass's M2 |
...
printed address into a miscompiled body. The `byval_agg` consumer bridge is the
fix location.
...
| ~~[defopaque-over-sym-skips-the-ptr-bridge](../archive/defopaque-over-sym-skips-the-ptr-bridge.md)~~ | medium | **RESOLVED 2026-09-13** (archived): `opaque_base_is_ptr` asked "is the base SPELLED with the word ptr" when the question is "is this carrier a pointer".  `cstr` and `Sym` are pointer-sized carriers (`const char *`, `const struct __tur_sym *`) and now take the pointer path, so the newtype c-names `void *` in both directions -- the fix the report argued for over a per-store bridge, since a per-store fix would have needed the read-back seam too.  The opaque ascribe seam also relabels a non-`void *` pointer inner through `intptr_t`, which launders the qualifier a `const char *` literal would otherwise discard.  Pinned by `tests/fixtures/defopaque-over-pointer-sized-carrier` (both carriers, both seams, plus the read across a call boundary); it fails pre-fix through run.sh's own -Wint-conversion ratchet, as the report predicted.  Original row: A `defopaque` newtype over `:Sym` stores the symbol POINTER straight into an `int64_t` slot: `int64_t r = ((const struct __tur_sym *)&__tur_sym_alice)` -- `incompatible pointer to integer conversion` [-Wint-conversion]. `tur check` is clean; the failure is in cc, and since AppleClang 21 makes that an ERROR it is a hard build failure on the macOS leg (see the macOS-strictness note) and a silent pointer-truncation risk elsewhere. The ptr bridge that other opaque-over-pointer bases get is skipped for `Sym`. Found 2026-09-11 executing crdt-spice-plan C1, which had flagged this exact shape as the one not yet covered by a fixture. |
...
| ~~[default-method-spliced-at-carrier-type](../archive/default-method-spliced-at-carrier-type.md)~~ | high | **RESOLVED 2026-09-13** (archived): `elab_definstance` now tracks which method slots it filled from the class's DEFAULT form and grounds that form's annotations at the instance -- a class-var parameter or return annotation is dropped so the slot keeps the inherited-and-substituted class signature (what a bare-parameter instance method already gets), and a COMPOUND class-var return (`: (Vec a)`) has its class variables substituted instead, since its shape is the class's word.  Scoped to spliced forms only: an instance's own annotation is its own word, and an instance head's free tyvar may share a name with a class type parameter.  `__inst_C_pick_float(double, double)` now, and `(g 2.5 7.1)` answers 2.5; BOTH halves of "currently unwritable" are closed.  Pinned by `tests/fixtures/default-method-class-var-result` (repro, int-instance control, an instance that writes the method, a fixed-return default).  stdlib's `max`/`min` stay constrained generic defns by preference, not necessity.  Original row: **Silent wrong answer.** A typeclass DEFAULT method whose declared types mention the class variable is spliced into each instance with its `: a` annotations intact and elaborated literally, so `C [float]`'s copy emits as `int64_t __inst_C_pick_float(int64_t, int64_t)` and converts its own arguments: `(pick 2.5 7.1)` through a generic answers 2. Dispatch is correct; the SIGNATURE is wrong. Dropping the annotations is not an option either -- the class var then appears only in the return and every call site is "cannot infer type for return-directed method" -- so such a default is currently **unwritable**. Defaults returning a fixed type (`: bool`, `: cstr`) are unaffected, which is every default in the tree today. Caught by the -Wfloat-conversion ratchet, not by a stdout diff (the CONCRETE call is correct). Workaround: a constrained generic defn, which is what stdlib's `max`/`min` use |
...
| ~~[float32-block-temp-widens-to-double](../archive/float32-block-temp-widens-to-double.md)~~ | low | **RESOLVED 2026-09-13** (archived): fixed at ELABORATION, one layer above where the report pointed -- the emitter types the block temp from the tail's type, and the tail was the literal's own default. `rc_narrow_float_literal_tail_to_return` retypes a float literal in tail position from the declared `: float32` result and carries it out through the `do`/`let` wrappers, beside the existing int-literal widen. `EX_IF` is deliberately not walked (its arms are already unified with each other). **The ratchet exclusion is now EMPTY, not merely narrowed**, swept clean across the corpus first (2974 passed, 0 failed); the knob is kept as a named one-liner so re-widening is one edit. Pinned by `tests/fixtures/float32-block-tail-literal`. Original row: A float literal closing a MULTI-expression `: float32` body gets a `double` temp and narrows on return; a single-expression body is fine. Values are right today (exact literals), but a non-representable one would round twice. Visible only under GCC, whose `-Wfloat-conversion` covers double->float where clang splits it out -- which is how the Linux leg found it. `tests/run.sh`'s ratchet excludes float-destination warnings so this does not fail the suite; **narrow that exclusion when this lands** |
| ~~[byvalue-struct-param-as-if-arm-derefs](../archive/byvalue-struct-param-as-if-arm-derefs.md)~~ | medium | **RESOLVED 2026-09-13** (archived): the decline that says "this arm already holds the merge temp's aggregate" (`emit_arm_is_byval_agg_var`) keyed off `emit_type_is_byvalue_sum`, which requires `n_ctors > 1` -- so a single-variant lowered `defstruct` never matched it and `emit_if_value` bridged the bare parameter carrier->concrete. Widened to `emit_type_is_byvalue_adt`; the real gate is the C-type-name equality that follows, so it removes bridges and never adds one. Pinned by `tests/fixtures/byvalue-struct-param-if-arm` (both broken shapes, the else-arm mirror the filing missed, the three that already worked, and the tail-recursive fold). Original row: A by-value struct PARAMETER used directly as an `if` arm emits `*(tur_adt_S *)(intptr_t)(x)` -- dereferencing the struct. `tur check` is clean, cc rejects it. Measured: parameter-vs-ctor and parameter-vs-call both break; call-vs-ctor and ctor-vs-ctor are fine, so it is the BARE PARAMETER as an arm, and wrapping it in any call (even identity) fixes it. A tail-recursive fold with the same shape compiles, so a fix must not assume the general `if` unifier covers everything. Hits any "transform or pass through" helper over a struct |
| [set-add-elem-hash-disagrees-with-set-member](set-add-elem-hash-disagrees-with-set-member.md) | medium-high | **ROOT-CAUSED 2026-09-13, still open.** Not two hash functions disagreeing: `set-add-elem__` is emitted as ONE carrier-typed body (`set_hyadd_hyelem_un_un`) with `__inst_Hash_hash_int` and `__inst_MapKey_mk_cmp_int` HARD-WIRED, and every element type is routed through it. So `mk-cmp` is baked too -- a `(Set cstr)` built this way is keyed by ADDRESS, not content, which the filing missed and which a two-literal probe hides (the compiler pools them). The trigger is the RECEIVER: `(set-add-elem__ (set-new) x)` leaves `A` abstract, while `(let [s : (Set Sym) (set-new)] ...)` mints a real monomorph and works -- annotating the receiver is a workaround. `A` never grounds because `call_collect_type_bindings` (elab_call.c:952) binds `A` to a tyvar from arg 1 and then ACCEPTS the later concrete actual without overwriting -- deliberate, with a recorded `hamt-delete` regression behind it, so the one-line overwrite is the fix that has already been tried and reverted. Reordering the params to ground `A` first turns it into a hard error instead (a fresh `(set-new)`'s open `(Set A)` does not unify forward), whose diagnostic prints both sides identically. Fix is in inference, two options in the file; NOT in stdlib/set.tur, which would cost the typed homogeneity check. Original row: `set-add-elem__` (the typed `[^Hash A ^MapKey A]` adder) computes a different hash than `(hash x)`, so elements it adds are counted by `set-count` but invisible to `set-member?` -- the set quietly behaves as empty. The explicit-hash `set-add` / `set-member?` macro pair agrees with itself and is fine. There is `set-add1` sugar but no `set-member1`, which is probably how the gap survived. A size-only assertion passes either way |
| [signal-compose-hand-rolled-vec-readers](signal-compose-hand-rolled-vec-readers.md) | low | `spices/signal/src/signal/compose.tur` hand-writes `__vec-get-i` / `__vec-len-i` as inline C that reinterprets the Vec layout (`{data,len,cap}`) by hand, justified by a comment claiming project mode auto-loads only `macros.tur`. **Stale:** `stdlib_autoload.c`'s list is shared by single-file and project mode and carries `vec.tur`; `spices/plot` and `spices/linalg` both call stdlib `vec-get` in project mode today. Costs interpreter coverage -- run-turi.sh PASS-skips any fixture whose program has a user inline-C block -- and puts a private copy of the Vec layout in a spice. Delete both, call `vec-get`/`vec-len` |
...
| ~~[turi-nested-class-method-call-picks-first-instance](../archive/turi-nested-class-method-call-picks-first-instance.md)~~ | high | **RESOLVED 2026-09-13** (archived): the driver's EX_CALL gates all ask about a type that is a TYVAR, and a receiver that is itself a class-method call has the REPRESENTATIVE's concrete type instead -- so no frame dictionary was consulted. `eval.c` now walks that receiver for the tyvar the inner call dispatches on and feeds the existing `frame_lookup_dict_tyvar` path; no new dispatch mechanism, and none of the retired head-name heuristics come back. Took TWO rounds: the same-class walk fixed the headline repro and the newtype silent wrong answer but left every lattice law wrong, because those are `(eq? (join x y) y)` -- a CROSS-class receiver. The walk accepts one, guarded by the inner method's declared class return being that class's type variable. That also corrects this report's own table, which recorded `(eq? (combine x y) (combine y x))` as ok on both back ends: it is ok in the MAIN file and wrong in a `load`ed module, where every stdlib law lives. All four `requires.compiled` markers removed as instructed, plus a new `tests/fixtures/turi-nested-class-method-dispatch`. run-turi 2065/0, run.sh 2975/0. Original row: The INTERPRETER half of the now-fixed compiled defect: `tur --interpret` resolves a NESTED class-method call in a constrained generic to the first-declared instance and dies with `unknown infix builtin '<'` -- a builtin the program never mentions, from the `int` instance it wrongly picked. **float is correct there, bool and cstr are not**, which is why the compiled report recorded turi as a reliable oracle for this class; it is not. Compiled path fixed 2026-09-11 in `emit_reresolve_disp_type`; that fix is emit-side and does not port. `tests/fixtures/typeclass-nested-method-call-float` carries a `requires.compiled` marker naming this -- remove it when this lands. **Raised to high 2026-09-11:** over `defopaque` newtypes it is a SILENT WRONG ANSWER, not a crash -- `law-associative?` answers `true` for a deliberately non-associative instance, certifying exactly what it was written to catch. Trigger measured as nesting + constrained generic, not cstr/bool. Also skips `typeclass-lattice-semigroup-monoid` and `typeclass-lattice-join-meet`  **Third symptom (a constrained generic passed as a FUNCTION VALUE) FIXED 2026-09-12** -- and its root cause was NOT this report's: interpreter frames chain LEXICALLY (`cl->captured` is NULL for a top-level defn), so a generic referenced as a value travelled without the caller's `TyvarBind`. Fixed in `eval.c`'s `EX_VAR` by re-homing such a closure onto a frame carrying that binding. Measured: this did **not** fix the four nesting fixtures below, so that symptom is a separate mechanism. |
...
| ~~jit-win-prelude-shadows-user-fn~~ | -- | **Resolved at filing; archived 2026-09-10** (it sat here with a fixed-at-filing row). A program defining a libm name (`log2`, `sqrt`, `floor`, ...) was called through JIT_PRELUDE_WIN's `double` prototype: the int argument goes in xmm0 while the callee reads rcx, so the program ran to completion and printed a pointer. Fixed by dropping a prelude declaration the program itself defines; `jit_prelude_win_shadowed` (`src/jit_engine.c:652`) and `tests/fixtures/jit-win-prelude-name-shadow` pin it. Its "Still open" half is now [jit-win-prelude-and-libc-names-drift](jit-win-prelude-and-libc-names-drift.md). See [../archive/jit-win-prelude-shadows-user-fn.md](../archive/jit-win-prelude-shadows-user-fn.md). |
| [jit-win-prelude-and-libc-names-drift](jit-win-prelude-and-libc-names-drift.md) | low (latent) | Nothing keeps `src/jit_win_prelude.h`'s 106 entry points and `mangle.c`'s `libc_names` in step, so a name added to the prelude re-opens the shadowing hole **silently** -- a wrong answer, not a build error. `jit_prelude_win_shadowed` closes the class, so it is not urgent. Fix direction: derive the prelude's names and assert at build time that they are a subset of `libc_names`, which is verifiable off Windows (widening `libc_names` instead changes shared codegen for a Windows-only defect) |
