...
| [direct-call-does-not-inherit-caller-tyvar-binding](direct-call-does-not-inherit-caller-tyvar-binding.md) | medium | The compiled path answers one question two ways: a constrained generic whose tyvar appears in NO parameter inherits the caller's binding when PASSED as a function value but not when CALLED. **Well-formedness SETTLED** (2026-09-12): alpha-renaming the callee's `[V]` to `[W]` used to change the answer on both back ends, so the inheritance was name CAPTURE; both paths are class-keyed now. **Direct half attempted and NOT landed**, but narrowed to one step: (1) the relay probe's `n_abi_bindings > 0` guard blocks a callee that pins nothing -- dropping it plus class translation makes `viadirect` specialize, verified; (2) `spec_match_bindings` needs the `(!abi_changes && instance_changes)` arm or the clones dedup; (3) **the blocker** -- the inner call is scanned with the correct enclosing spec but never reaches the intern, and the exit point is unpinned (it takes none of the instrumented returns either). Both back ends currently pick DIFFERENT wrong instances (Gmax vs Gsum), which argues the answer is arbitrary rather than chosen. |
