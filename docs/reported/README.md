...
| [any-cannot-recover-a-capturing-closure](any-cannot-recover-a-capturing-closure.md) | medium | A fat `{thunk, env}` closure -- a lambda over an outer binding, or a partial application -- interns a different `any` box id from a bare fn of the same signature, so `is?` is false and `cast` panics compiled, while the interpreter (no fat/bare split) returns 42. Deliberate, since calling one as the other segfaulted, but it means no capturing closure round-trips through `any` compiled -- a capability divergence, and a portability trap in the worse direction (works interpreted, panics compiled). Fix is to fatten every fn payload at the widen via the existing EX_FN_TO_FAT shim; the open question is ownership of the per-widen fat box, which is why it is not folded into the row above |
