...
| [interpret-ignores-a-malformed-manifest](interpret-ignores-a-malformed-manifest.md) | low-medium | `tur --interpret` does not read `build.tur` at all, so a manifest the compiled path refuses to continue on (TUR-E0624, exit 1) is silently ignored and the program runs as if no manifest existed, exit 0. Not `#lang`-specific: an unterminated list in the manifest -- the plainest malformed manifest there is -- gets the same silent pass, which is what shows this is about manifest handling generally. Matters because the manifest is what puts the spice's own `src/` and its `:spices` deps on the module search path, so a typo in `build.tur` surfaces under `--interpret` as an unrelated "unknown name" downstream with the real cause never mentioned. The discovery-and-validate step that produces `PKG_MANIFEST_MALFORMED` runs from the compiled entry points and the per-file commands but not from the `--interpret` one. Found while adding `errors/lang-trailing-token-manifest` during the `#lang` layer decommission; pre-existing and unrelated to it |
