...
| [option-rc-payload-legal-to-return-illegal-to-build](option-rc-payload-legal-to-return-illegal-to-build.md) | low-medium | `(Option rc<A>)` is accepted as a return type, is buildable from inline C, and ships in stdlib (`weak/upgrade`) -- but `(some r)` where `r : rc<A>` is rejected "cannot store an owning value (rc) in a collection". One of the two is wrong. **A decision, not a diagnosis**: the mechanism is fully located (`own_carry_for_arg` in `elab_call.c` is an allowlist; `some`/`ok`/`err` are simply not on it), and the report lays out three answers with the tradeoffs -- allow it as a MOVE (BORROW, not RETAIN, since nothing releases an Option's payload), make the return type illegal too, or give Option drop glue for owning payloads. Recommends the first now and the third eventually. Split out 2026-09-02 from `inline-c-option-carrier-box-leaks`, where it was riding as fix direction 2 |
| [inline-c-option-carrier-box-leaks](inline-c-option-carrier-box-leaks.md) | medium | an Option/Result built with the preamble builders inside inline C allocates a 16-byte carrier box nothing frees automatically -- and that is the form the guide and CLAUDE.md recommend. **Documentation half done 2026-09-02**, and two of the report's own claims corrected by measurement: the box IS ownable (`option-free`/`result-free` on the returned carrier is clean), and the arc-style split does NOT fix an erased-path site (a Turmeric `(ok x)` in a generic function allocates the same box) -- so there is nothing to convert at either stdlib site. `result-bimap`'s leak is now measured at 16 bytes/call (was unprobed). New hazard found: `result-free` on a concrete BY-VALUE monomorph is a bad-free, so "free what the builder returned" needs the erased-vs-monomorph rule the guide now carries. Stays open for direction 1 / the carrier campaign; direction 2 (the `(some rc)` inconsistency) is located at `own_carry_for_arg` but is a real ownership decision, not a mechanical fix |
