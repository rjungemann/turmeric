...
| --- | --- | --- |
| [sr2-carrier-seam-rotted](sr2-carrier-seam-rotted.md) | medium | `TUR_SR2_APP_SUM_BYVALUE=0`, the bisection hatch left behind when `parametric-sum-byvalue` graduated, no longer produces correct programs. Three defects, one fixed. **FIXED 2026-09-04**: the hatch aborted the compiler unless `TUR_OPTION_NICHE=0` moved with it -- `sr3_option_niche` now follows its substrate down, so the hatch is a one-variable switch again. Still open: `sum-passthrough-param-not-dropped` is a hard C compile error on the carrier path regardless of the niche; `option-niche-vec-closure-cmp` is a silent wrong answer on the both-off diagonal, which is how it hid from two one-axis harnesses. Default path unaffected throughout. Found by restoring `tests/run-sr2-seam.sh`, retired at graduation |
