...
| [defopaque-over-sym-skips-the-ptr-bridge](defopaque-over-sym-skips-the-ptr-bridge.md) | medium | `(defopaque R :Sym)` stores the interned symbol POINTER into an `int64_t` slot: `incompatible pointer to integer conversion ... 'const struct __tur_sym *'`, which clang >= 21 rejects outright. `tur check` is clean. A one-field `(defstruct R [name : Sym])` is fine -- v0.46.1's `4ff9ad724` added the Sym rows to the defstruct/defdata tables but not the opaque path, whose `opaque_base_is_ptr` keys on the base being SPELLED a pointer and Sym is not. Same representation judgement as global-def-store-misses-int-ptr-bridge |
