...
| Report | Severity | One line |
| --- | --- | --- |
| [windows-subprocess-and-shared-lib-gaps](windows-subprocess-and-shared-lib-gaps.md) | medium (was high) | Filed as "`tur install` / `fetch` / `new` / `build --shared` / REPL spice loading all fail", read-verified by audit. **Re-checked 2026-09-26: one item left.** `install` passes `run-install.sh` on Windows 34/34, `fetch` clones, locks and integrity-checks against a `file://` remote, `new` and REPL spice loading were verified end to end 2026-09-04, and `build --shared` emits a `.dll` the loader agrees on. Every shell-string site is converted (the report's own "still POSIX-only" list had fallen behind). **Open: section 3** -- `tur repl --engine jit` builds its module shadow directory with `symlink()`, an `ENOSYS` stub on Windows |
| [windows-httpd-async-limit-hangs-on-ci](windows-httpd-async-limit-hangs-on-ci.md) | low | Hung on GitHub's Windows runners while passing locally. **Diagnosed 2026-09-04**: reproduces on two cores, and the process is spinning, not blocked. The reactor half (source slots never reused -- unbounded growth and O(ever-created) polls, not test-only) is **fixed**. **Open: the fixture half** -- the handler's `(while (< busy 2) ...)` gate never opens if an over-cap client drops out without being counted. Still skipped via `requires.win-concurrent-loopback` |
| [jit-windows-support-spike](jit-windows-support-spike.md) | research | Spike EXECUTED: the Windows JIT builds and runs. The **lazy tier is fixed** (three defects in MIR's win64 wrapper assembly, rjungemann/mir#3) -- the default mode went 0/5 to **2637 passed / 68 failed** on the full corpus. The JIT longjmp is fixed too (win64 SEH cannot unwind JIT frames -- `STATUS_BAD_FUNCTION_TABLE`, not the fiber-stack cause this report recorded). Both remaining fixtures are fixed as well -- `path-string` was [jit-c2mir-implicit-decl-truncates-pointers](../archive/jit-c2mir-implicit-decl-truncates-pointers.md) and `cps-backend-nil-delegated-call` was [jit-win-prelude-shadows-user-fn](../archive/jit-win-prelude-shadows-user-fn.md) -- so the corpus is **2702 passed / 0 failed / 70 skipped**. What remains: `__va_start` on the whole-preamble fallback (uninvestigated), and the `windows-jit` CI job still runs only a hello-world smoke test -- its stated reason, a red corpus, stopped being true on 2026-09-05 |
