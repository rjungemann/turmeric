...
| [struct-param-mutation-backend-divergence](struct-param-mutation-backend-divergence.md) | medium-high | `(set! (.f p) v)` on a BY-VALUE struct parameter is a dead store compiled (`cc -O2` deletes it outright) and a write-through in turi, with no diagnostic either way. `rc<Struct>` works in both and `&Struct` is already rejected -- so the language has the right answer and just accepts a wrong spelling silently. One in-tree fixture (`catch-box-user-sink-confines`) has the latent bug today |
