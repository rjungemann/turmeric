...
| --- | --- | --- |
| [saffron-dynamic-surface-pass](saffron-dynamic-surface-pass.md) | medium (open items) | Differential pass over the Saffron surface (2026-09-09): 11 high and 10 medium findings plus lows, each with a repro. 2026-09-10: H1-H6, H8-H11, M3-M6, M8, M10 resolved and pinned (struck through in the file), and M2 resolved on the INTERPRETER only; still open: H7 (fn-typed seam is a representation gap), M1, M2's compiled half, M7, M9, and the lows. H7 and M2-compiled are the same shape -- a Saffron `tur_tagged_t` that does not fit the representation the typed path already chose -- and for both the cheap fix yields a silent wrong answer, so each is parked behind a clean decline. `tests/saffron-fuzz-src.py` avoids the open shapes via its KNOWN table and pins them with `--known-probes`. |
