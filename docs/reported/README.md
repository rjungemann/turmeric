...
| [match-arm-binder-in-any-monomorph-typed-as-carrier](match-arm-binder-in-any-monomorph-typed-as-carrier.md) | medium | A user-defined `(defdata Box [a] (MkBox a))` at `(Box any)` emits uncompilable C: the monomorph's struct field is correctly a 16-byte `tur_tagged_t`, but the match arm binder takes its C type from the GENERIC field type (a bare tyvar -> the int64 carrier) and emits `int64_t x = (int64_t)__scrut.as.MkBox._0;` then assigns it back to a `tur_tagged_t`. The specialization is half-applied -- layout substituted, binder not. **No Saffron required**: the repro is plain Turmeric and fails identically with the seam fix reverted. Surfaced by that fix, which moved this shape from a runtime panic to a compile error. `(Vec any)` is unaffected -- collection elements are malloc'd boxes, so their slot is a pointer, never a by-value tagged field |
