...
| [wide-payload-sum-monomorph-box-has-no-owner](wide-payload-sum-monomorph-box-has-no-owner.md) | medium | SR2a's "a concrete monomorph flows by value, so there is no box to own" holds only for payloads <= 8 bytes. A WIDER by-value payload is stored in the arm as a POINTER (the B4 wide-element rule), so the SPECIALIZED ctor mallocs it -- `ok__spec__...(Rational x)` does `malloc(sizeof Rational)` -- and nothing frees it. DEFAULT path, no experiment. Measured: 11 fixtures emit the shape, 9 leak with a `*__spec__*` ctor in the trace, 465 B. Unlike RM1's residue the ownership is unambiguous (the ctor mallocs a fresh copy), so drop glue is the fix that scales -- blocked on `elab_effects.c:30`'s `n_ctors == 1` gate |
