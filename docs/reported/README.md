...
| [fmap-over-byvalue-struct-element-passes-the-struct-as-int64](fmap-over-byvalue-struct-element-passes-the-struct-as-int64.md) | medium | **Filed 2026-09-09.** `(fmap (some (Pt 1.25 2.5)) sw)` with `Pt` a two-float defstruct fails in C inside the by-value spec's OWN body: `incompatible type for argument 2 of '(tur_adt_Pt (*)(void *, int64_t))g.fn'`. The spec is half-specialised -- the mapper's RESULT position was substituted to `tur_adt_Pt` but the ARGUMENT kept the int64 carrier, while the value passed is the struct just matched out of the container. Identical for a global defn, a lambda and a forwarded parameter, and on the pre-fix build, so it is not the argument packing. An `any` element specialises both positions and works; a float element takes the carrier base and works. The interpreter answers 2.5 |
