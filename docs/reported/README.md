...
| [cmake-dep-path-base-dir-inconsistent](cmake-dep-path-base-dir-inconsistent.md) | medium | `:path` is written relative to `<spice>/cmake/` (every spice does this, and says so) but is absolutized against `<spice>/` when collected transitively -- one level too high, so `add_subdirectory` points at a directory that does not exist. `tur fetch` and `tur build <dir>` also disagree for a spice's *own* dep. Found behind the doubling fix; three combinations, and they do not agree |
| [transitive-cmake-deps-collide-on-duplicate-targets](transitive-cmake-deps-collide-on-duplicate-targets.md) | medium | every workspace sibling's `:cmake-deps` are merged into one generated CMake project, so two siblings providing the same target name collide: `add_library cannot create target "glfw"`. `opengl` fetches glfw; `raygui` pulls raylib, which *vendors* glfw. Neither spice is at fault and neither declares the other's dep -- the merge creates it. The conflict detector keys on manifest dep names, a different namespace from CMake target names |
| [cmake-deps-fail-on-cmake-4-policy-floor](cmake-deps-fail-on-cmake-4-policy-floor.md) | medium | CMake 4 removed `cmake_minimum_required` floors below 3.5, so a dep that has not updated (hiredis) aborts the entire configure and no dep builds. `tur` never passes `-DCMAKE_POLICY_VERSION_MINIMUM` -- though turmeric's own bootstrap does. CI runners still pin CMake 3.x, so this is a local failure that will arrive in CI on its own schedule |
| [transitive-path-cmake-dep-absolutized-then-reprefixed](transitive-path-cmake-dep-absolutized-then-reprefixed.md) | medium | a `:path` cmake-dep reached **transitively** is absolutized by the resolver and then re-prefixed by the emitter, giving `add_subdirectory(".//Users/...")` (or `<project_dir>/<abs>` under `tur build <dir>`). CMake configure fails and no dep builds. A spice's *own* `:path` dep is fine -- only the transitive one breaks. Blocks `spices/opengl`; pre-existing on `423f6546`. Compounds with `spices-ci-fetch-failure-downgraded-to-warning`, which hides the cause |
| [cmake-dep-shared-target-links-without-rpath](cmake-dep-shared-target-links-without-rpath.md) | medium | a `:cmake-deps` `:targets` entry that is a **shared** library links by artifact path with no `-Wl,-rpath`, so the binary links clean and dies at startup with `dyld: Library not loaded: @rpath/libz.1.dylib`. Pre-existing (the old `-l` line hit it too, and it is the original zlib finding) but now reachable whenever a shared target is named; for the both-targets declaration it moved a link-time error to a runtime one. Nothing in the failure points at `build.tur` |
