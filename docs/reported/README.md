...
| [eq-two-pair-monomorphs-sharing-a-component-cross-bind](eq-two-pair-monomorphs-sharing-a-component-cross-bind.md) | medium | a `(Pair (Option String) int)` and a `(Pair (Option String) (Option String))` each work ALONE; in one program the `Eq[Pair]` spec bodies cross-bind, and the `.snd` dispatch of one is handed the FIRST component's unboxed Option payloads. Needs two Pair monomorphs SHARING a component -- one sharing nothing does not reproduce. The damage is in the spec BODY, not the key (a `(Pair (Vec int) int)` body contained `tur_opt_value_checked` calls). Distinct from the row below, which measurably improves this repro (5 warnings -> 2) without closing it |
