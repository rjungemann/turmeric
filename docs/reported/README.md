...
of it. They look like one defect and are two: the first is fixed, and fixing it
left the second exactly where it was.
...
| [region-bracket-lost-when-bt-scope-specializes](region-bracket-lost-when-bt-scope-specializes.md) | medium | Under `--enable=regions`, a `bt-scope` whose result is a by-value record emits ZERO `tur_region_push()` -- not "pushes then retires", no bracket at all. Not the static walk (`region_type_reaches_node` accepts a non-heap 2-int ADT); the call takes the `cps->cps` arm, and only `cps->direct` carries the push. Right answer, silent no-op. Blocks the 312 B of `re.tur`'s `re-find-from` spine that is otherwise the textbook region (see [rm2-spine-residue-categorized](../artifacts/rm2-spine-residue-categorized.md)). **Not closed by the row above** -- its filing predicted the bracket would ride along on that fix; it did not, and this was checked after the fix landed rather than assumed. The callee now resolves correctly and the push is still absent, because the `cps->cps` arm has no bracket in it at all |
