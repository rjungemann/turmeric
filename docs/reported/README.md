...
| Report | Severity | One line |
|---|---|---|
| [let-alias-of-fn-param-captured-in-lambda](let-alias-of-fn-param-captured-in-lambda.md) | medium | `(let [g f] ... (fn [] ... g ...))` where `f` is a fn-typed PARAMETER (so it arrives as the fat-closure handle). In CALLEE position the lifted body names `g` directly instead of reading it from the env -- emitted C says `'g' undeclared`, so `g` was never counted as a capture. In ARGUMENT position it compiles and SEGFAULTS. Both pre-existing: they reproduce against a compiler without the fat-alias declaration fix that landed beside this filing (that fix removes an additional `-Wint-conversion` on case 2 and leaves the SEGV). Note `stdlib/logic.tur`'s `st-bind` runs the SAME alias-and-capture-in-argument-position shape correctly, so case 2 is not "argument position is broken" -- the working/failing pair is a cheap bisection and the report lists the candidate differences |
