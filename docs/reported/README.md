...
| Report | Severity | One line |
| --- | --- | --- |
| [typeclass-default-methods-do-not-work](typeclass-default-methods-do-not-work.md) | medium | **Filed 2026-09-09.** The typeclass guide documents default method bodies with a worked example; none of it works. The guide's exact Ord example fails `unknown function or operator 'lt?'`, the dotted sibling call fails `no typeclass method found` (the default body is elaborated at the class tyvar before any instance exists), an instance that OMITS the defaulted method is rejected `expected 1 method implementations, got 0` before any fallback -- and merely WRITING a default breaks a class whose instances all implement it. Plain Turmeric; found answering saffron D8 question 2, which is blocked on this. |
| [duplicate-instance-silently-drops-a-user-definstance](duplicate-instance-silently-drops-a-user-definstance.md) | low-medium | **Filed 2026-09-09.** A user `(definstance Eq [int] ...)` is a silent no-op: stdlib's autoloaded instance was first, and the idempotent re-instance guard in elab_definstance (written for repeated `(load ...)`s of the SAME file) drops any later instance with the same type suffix without a word. Consistent on both back ends and under S9's dynamic dispatch because there is only ever one instance to find. Defeats the file's own T1 intent that a user instance shadows a stdlib one. Found answering saffron D8 question 4, which is therefore not a dispatch question. |
...
| Report | Severity | One line |
| --- | --- | --- |
| [local-fn-value-into-rank2-slot-gets-a-by-name-wrapper](local-fn-value-into-rank2-slot-gets-a-by-name-wrapper.md) | medium | **Filed 2026-09-09.** `(defn ap [o : (Option any) f : (fn [any] any)] (fmap o f))` fails `'f' undeclared`: the call resolves to the by-value spec, and the rank-2 path wraps `f` via make_poly_wrapper_ex, which synthesises a FILE-SCOPE `__poly_N` that calls the inner binding BY NAME -- right for a global defn, meaningless for a parameter. The float version works (it reaches the carrier base and passes the tur_poly_fn_t straight through); a CAST expression works (fat pass-through). Plain Turmeric; found building D8 Q3, whose witness sidesteps it with the cast form. |
