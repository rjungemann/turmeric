...
| [any-widen-stored-in-an-adt-field-has-no-owner](any-widen-stored-in-an-adt-field-has-no-owner.md) | medium | **Narrowed three times; the slug is now misleading and the heading is the accurate one.** The ADT-FIELD case is fixed (an `:any` field is owning, `__tur_any_drop` in the glue), and it was never the root: attributing `saffron-higher-order`'s leaks to their sites gives direct roots in ARGUMENT and RETURN position plus field/spine boxes that leak only because their holder does. **2026-09-18 closed the argument-position root and fixed a USE-AFTER-FREE in the same machinery.** (a) `box_uses_confined` answered `EX_DYN_CALL` with a bare `return false`, which is right about a value flowing INTO a dynamic call and wrong about the mere presence of one -- it disqualified every parameter of every Saffron higher-order body, so `lmap`'s `xs` read as retained because a SIBLING subexpression called `(f h)`. Now checks the call's operands. 680/17 -> 520/13. (b) The deep `__tur_any_drop` (2026-09-14) outran `returns_fresh_any`, which only ever established the BOX is fresh: `(defn wrap [x] (Cons x (Nil)))` qualified, and the consumer's drop freed a spine its caller still owned (`heap-use-after-free`, silent without a sanitizer). Freshness now requires a payload that owns nothing. (c) `tests/run-leak-check.sh` keyed `known-leak` off the ASan exit code, which is 23 for EVERY ASan error -- so it would have printed that use-after-free as KNOWN and stayed green. A marker now excuses a leak and nothing else. What REMAINS is the return-position widen: the callee's `any` result is heap by necessity and the caller never claims it; a deep drop is unlicensable because the elements came back from an opaque `(f h)`. Prerequisite for S6 **2026-09-18 (second pass) found the same hole in TWO more places and fixed both.** `any_expr_is_owned_temp` answered its WIDEN base case with `!frame_box`, so `(peek (:: one any))` deep-dropped a borrowed local's spine (`double free detected`); and the localowned SCOPE drop had no idea a widen is a hand-off at all, so `(let [inner (Cons 1 (Nil)) outer (Cons inner (Nil))] ...)` freed inner's spine twice -- six lines, both dialects, and not reached by any freshness rule. The move discipline the scope drop already relies on gained a fourth guard (`is_binding_widened_to_any`), narrowed to widens the CONSUMER can store (keying on the widen alone cost `saffron-higher-order` its spine, 13 allocations back to 17). Also measured: the residue grows linearly in BOTH data size and iteration count (~22 allocations/turn, flat), so a Saffron loop over data grows RSS without bound. And a tag-based spine drop -- the obvious cheap direction -- is UNSOUND and should not be attempted: a list of lists emits the element and the spine with the identical tag. A static producer-keyed spine drop is direction 3.
...
| [colored-generic-tyvar-elemented-param-sig-rejects](colored-generic-tyvar-elemented-param-sig-rejects.md) | low | A colored generic whose parameter is a tyvar-elemented application (`[E] [r : (Result int E)]`) is `SIG-REJECT`ed by `sig_slot_ok` -- deliberately, since the mono-template machinery relies on the generic BASE sig-rejecting -- and with nothing pinning `E` there is no clone to route to, so `(peek (ok 1))` under a handler is "no lowering here" while `--interpret` prints 1. The pinned-arm control lowers. Filed 2026-09-16 from the eviction report's resolution; it is also why the CPS deferred-drop tail arm is unreachable. Direction: mint a clone with the unpinned arm defaulted to the carrier, as the direct emitter already does |
...
| Report | Severity | One line |
|---|---|---|
| [pwa-overlays-ignore-ios-safe-area](pwa-overlays-ignore-ios-safe-area.md) | high | `position: fixed` resolves against the viewport, so every overlay escaped the safe-area padding `#app` reserved for the black-translucent status bar: the docs pane's 44px topbar sat inside a 59px inset with **every** control that leaves the pane in it -- close, Contents, search -- so an installed-PWA reader who opened a doc had no way back. Fixed, with tests. The companion "black area at bottom" is the same inset at the other end (59px of band, exactly `safe-area-inset-top`, from `#app` sizing itself with `100dvh`); its fix pins the shell instead of sizing it. **The `100dvh` mechanism is no longer inferred:** measured on a second screenshot (iPhone 16 Pro, inset 62) the app chrome ends at CSS y 811 and 874 - 62 = 812, so in an iOS standalone web app `100dvh` is the screen height minus the top inset, exactly as the fix assumes. Still not confirmed *working* on a device -- the reporter's phone cannot fetch the build, which is [pwa-installed-build-cannot-be-updated](pwa-installed-build-cannot-be-updated.md). Note 37d44e55d already tried to fix that band by dropping the *bottom* inset -- 34px against a 59px band -- and the report came back |

| Report | Severity | One line |
|---|---|---|
| [pwa-installed-build-cannot-be-updated](pwa-installed-build-cannot-be-updated.md) | high | An installed PWA has no way to notice or apply a new build: `grep -c "controllerchange\|\.update()" web/main.js` is **0**, so a resumed standalone app (which fires no navigation) never checks, and a new worker that does take over never causes a re-render. **Force update was a placebo on iOS** -- it unregisters and empties Cache Storage correctly, then calls `location.reload()`, which WebKit can answer from its own HTTP/page cache that neither step touched; its test asserts the reload *hook fired*, never that a fetch happened, so the gap was invisible on desktop. One phone sat months behind, immune to relaunch, Force update, icon delete/re-add and clearing Safari data. Fixed here: only the recovery lever -- `sw-kill.js`, which had never been run and whose `activate` did async work with no `event.waitUntil()` (the browser could kill it before one cache was deleted), and which would have put a PWA in an **infinite reload loop** when deployed at `/sw.js`. Now tested against a real worker and real caches, shipped by `TUR_SW_KILL=1`, runbook in guides. The update lifecycle itself is still open |
