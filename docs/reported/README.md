...
| [cps-self-tail-call-relies-on-sibling-call](cps-self-tail-call-relies-on-sibling-call.md) | medium (low since 2026-09-26) | **Self tail calls resolved 2026-09-26**: a CPS function's self tail call that hands on its own `__kont` is a backedge (params rebound, `goto __tur_cps_self`), so `for-each`, `map`, `member`, `delay-force` and a loop through a variable run at `-O0`; pinned by `tests/fixtures/r7rs-cps-loops-unoptimized`. Still open: a tail call to ANOTHER CPS function (mutual recursion through procedure variables) is a C tail call, which overflows below `-O2`. Original row: A CPS (`__cps`) function's self tail call is emitted as `return f__cps(...)` (emit_cps_ir.c:6920), never a backedge, and gcc 13 has no `musttail`: a million-element Scheme `for-each`, `map`, `member` or `delay-force` stream segfaults at gcc `-O1`, passes at `-O2` and interpreted |
...
| [let-bound-generic-call-result-in-generic-truncates](let-bound-generic-call-result-in-generic-truncates.md) | high | The generic-FUNCTION twin of the row above, and needs no typeclass: `(defn wrap [A] [x : A] : A (let [y (gid x)] y))` returns `9` for `9.75`, and at a by-value aggregate `A` the emitted C does not compile (`int64_t y = <Option struct>`). elab_call.c types a call instantiated to the caller's `A` as `int`. Keeping the tyvar there failed 16 fixtures (the gate cannot tell a caller's tyvar from an unbound one, and several emitter paths rely on the `int`); the report records which. Fuzzer shape `gid_let`, `--emit-known` only |
