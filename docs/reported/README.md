...
| [constrained-generic-as-fn-value-collapses](constrained-generic-as-fn-value-collapses.md) | medium-high | **SILENT WRONG ANSWER**, for a shape currently worked around by duplicating code. A constrained generic passed as a FUNCTION VALUE (`(fold vjoin a b)`) collapses every instantiation onto one instance; called directly it is correct at each. This is what forces `crdt/ormap` to carry two near-identical folds differing in one line. Three layers must line up and the writeup measures all three: the enclosing generic must specialize (fn-typed ARGUMENTS are not followed, and both probes miss the variable because a poly-slot argument is wrapped in `EX_POLY_WRAP`); the fn value needs per-instantiation clones (`emit_abi_scan_fn_values` bails on `!abi_changes`, and same-carrier newtypes share an ABI); and **the remaining gap** -- the `tur_poly_fn_t` literal names ONE elaboration-time wrapper hardcoding one clone, and `raw_name_for_binding`/`atom_var` consult no specialization. Steps 1-2 were implemented and reverted: inert for every fixture but fixing nothing alone. Fixture owed. |
