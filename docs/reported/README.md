...
| Report | Severity | One line |
| --- | --- | --- |
| [vec-of-parametric-sum-monomorph-ice](vec-of-parametric-sum-monomorph-ice.md) | medium | `(vec-of (Yep 8) ...)` over a parametric two-variant sum ICEs at the let binder on the DEFAULT path -- repr-shadow catches the Vec registration saying typed heap pointer while the binder says carrier. Predates the SR work (reproduced at the merge base); on SR2's critical path because `vec<option<T>>` is this shape |
