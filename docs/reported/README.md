...
| [region-escape-through-unhooked-stores](region-escape-through-unhooked-stores.md) | medium | **Filed 2026-09-06 (region-lock-hardening).** The residue of widening the region runtime lock from "the result word" to "every word that leaves the generation": a node stored by a path the `TUR_REGION_NOTE` hook set does not cover -- a user inline-C body writing into its own `malloc`'d cell, or a stdlib primitive outside the hooked list (`sized-*-set!`, `fiber-local-set!`, channels, the serializer) -- still rewinds under the reader. The three shapes that motivated the widening (store into an outer vec, erased `:int` field in an admitted record result, `(Vec int)` of erased nodes) are fixed and pinned on both arms by `region-escape-via-store` / `region-escape-via-erasure`. Rule for new primitives is in CLAUDE.md; a top-level panic jam (latent, unreachable today) is recorded alongside |
