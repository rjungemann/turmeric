...
| [phantom-constrained-generic-base-body-picks-aggregate-instance](phantom-constrained-generic-base-body-picks-aggregate-instance.md) | medium-high | A constrained generic over a PHANTOM type parameter fails to compile -- TUR-E0295, naming a type the function never mentions -- whenever the class has ANY by-value aggregate instance in scope. The function need not be called: the GENERIC BASE body has to give the phantom var some type, picks a representative instance, and reinterpreting an aggregate as a one-word carrier is what E0295 rejects. Adding an unrelated instance elsewhere breaks a function that compiled yesterday. **Blocks crdt-spice-plan 2.3's constrained `(ORMap V)`** -- entries live in a HAMT of carriers so `V` is necessarily phantom, and `JoinSemilattice` has `DotContext`/`GCounter`/`ORSet` instances. Survives both same-day dispatch fixes; distinct from both. 8-line repro; fixtures owed. |
