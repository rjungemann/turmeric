...
| [any-fn-tag-does-not-discriminate-signatures](any-fn-tag-does-not-discriminate-signatures.md) | **high** | An `any`-boxed function carries the bare `TY_FN` TypeKind, so `is?` lowers to `TUR_GETTAG(v) == 7` and answers true for EVERY function type -- wrong parameter types, wrong arity. `cast` then returns a callable typed however the caller asked, and calling it is undefined behaviour (`(cast int->int-fn (-> cstr cstr))` called with a string printed garbage). The back ends also disagree on the `is?` answer. Found fixing the row above, which makes it easier to reach but did not cause it |
