...
| [forall-dict-float-result-truncated](forall-dict-float-result-truncated.md) | **high** | A float result dispatched through a mode-B dict clone is **silently truncated**: the clone returns the carrier via `(int64_t)(intptr_t)__ps`, a numeric conversion, so a method declared `: float` returns 2.5 as 2 and 7.1 as 7 -- no warning, no diagnostic. Static dispatch of the same method is correct. Carrier-shaped receivers only, so it is independent of the by-value defect above. Every float/carrier crossing elsewhere in the emitter bit-reinterprets through a union; this one converts. Went unnoticed because no `forall-dict-*` fixture declares a float result, and `7.0` would have looked right |
...
| [any-coercion-not-driven-by-expected-type](any-coercion-not-driven-by-expected-type.md) | medium | An `: any` annotation does not drive widening at a `let` binding or an `if` join. `(let [x : any 42] (type-of x))` leaves `x` an `int`; `(defn f [b] : any (if b (Some 7.1) 42))` is "mismatched types" though both arms would widen to the declared `any`. A branch facing an ALREADY-`any` sibling widens fine, so the machinery is there -- the expectation just is not a widening target |
