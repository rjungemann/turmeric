...
| --- | --- | --- |
| [saffron-dynamic-surface-pass](saffron-dynamic-surface-pass.md) | medium (open items) | Differential pass over the Saffron surface (2026-09-09): 11 high and 10 medium findings plus lows, each with a repro. 2026-09-10: H1-H6, H8-H11, M4-M6, M10 resolved and pinned (struck through in the file); still open: H7 (fn-typed seam is a representation gap), M1-M3, M7-M9, and the lows. `tests/saffron-fuzz-src.py` avoids the open shapes via its KNOWN table and pins them with `--known-probes`. |
