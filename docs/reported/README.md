...
| [saffron-unannotated-param-container-cast-panics](saffron-unannotated-param-container-cast-panics.md) | **high** | `(defn pick [v] (vec-get v 0))` in a Saffron file panics `cast: any holds a different instantiation of Vec` compiled, and answers correctly under `--interpret` -- passing a vector to a function, with the parameter unannotated, which is the entire point of the dialect. D5's boundary cast exempts a bare `TY_TYVAR` expected type as having "nothing to check against"; `vec-get`'s `(Vec A)` is a `TY_APP` whose ARGUMENT is a tyvar, so it slips through and checks against an instantiation the tyvar never determined. The interpreter head-matches parametric targets on purpose, so it never sees a mismatch. Missed because every Saffron container fixture either annotates the param (the prelude declares `v : (Vec any)`) or calls the accessor inline where the receiver type is static. **Direction 2 (ground the open args to `any`) was ATTEMPTED and REVERTED**: it removes the panic and produces a WRONG ANSWER -- the element's box address instead of its value -- because the seam's substitution is not visible to the result-tyvar grounding, so the result collapses to the int64 carrier and the return position widens the BOX POINTER. Measured down to the one differing line of emitted C; the fix belongs in the tyvar grounding, not the seam |
