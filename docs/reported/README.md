...
| [r7rs-procedure-body-forward-reference](r7rs-procedure-body-forward-reference.md) | medium | Filed 2026-09-25. `(define (f) y)` before `(define y 1)` is "unbound symbol 'y'" on both back ends; R7RS allows it. A `defn` is pre-declared, a plain variable is not; fix is to pre-declare every top-level defined variable of a Scheme program as an `any` global |
