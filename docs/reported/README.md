...
| Report | Severity | One line |
| --- | --- | --- |
| [vec-get-byval-struct-element-returns-carrier](vec-get-byval-struct-element-returns-carrier.md) | medium | `vec-get-byval` on a `(Vec P2)` / `(Vec (Pair int int))` -- a Vec of a by-value struct -- does not compile: its spec clone returns the int64 element-box carrier where the aggregate is expected, because the `(:: <carrier> A)` reinterpret inside the spec body emits a bare return with no unbox. `(:: (vec-get v 0) P2)` at the call site works. Regions on or off. Found writing `region-scope-parametric` |
