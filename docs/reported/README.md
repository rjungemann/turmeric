...
| [interp-native-ctor-loses-adt-name](interp-native-ctor-loses-adt-name.md) | medium | `type-of` on an `any` built by the stdlib `some` answers `"Option"` compiled and `"Some"` interpreted: `some` is a native override whose value has no ctor->ADT link, so `turi_any_named_type` falls back to the constructor name. The ADT ctor directly, and a user generic wrapping it, both agree. Scope beyond `some` is unestablished -- every native ADT constructor is a candidate |
