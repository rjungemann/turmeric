...
| Report | Severity | One line |
| --- | --- | --- |
| [poly-fn-with-any-parameter-is-called-with-the-int64-carrier](poly-fn-with-any-parameter-is-called-with-the-int64-carrier.md) | medium | **Filed 2026-09-09.** `(fmap o (fn [x : any] : any ...))` over an `(Option any)` emits uncompilable C: the closure's thunk takes the 16-byte `tur_tagged_t`, the `tur_poly_fn_t` shim the by-value spec calls it through passes the 8-byte carrier. Plain Turmeric, static dispatch; `vec-map` over `(Vec any)` works because it never goes through a poly-fn. The single blocker for keying Saffron's dynamic HKT dispatch on the head constructor (D8 question 3): the row-generation rule is designed and buildable, and the interpreter already answers 42, but the compiled path has nothing sound to call. |
