...
| [region-walk-refuses-every-adt-result](region-walk-refuses-every-adt-result.md) | low | `region_type_reaches_node` refuses on a ctor field with a NULL `full_type`, whose comment describes only the self-recursive spine -- but `full_type` is also NULL for an ordinary `:int`, so `(RIP :int :int)` is refused at field 0 and every ADT result retires instead of rewinding. Deciding by `kind` is unsound and was measured so: a genuine `:int` and a carrier-erased `:MB` ADT field BOTH report `TY_INT` with no `full_type`, and accepting on kind reclaimed a live mutually-recursive spine. The declared type IS reachable -- `c->field_forms[fi]` is populated for plain defdata, contrary to its comment -- so this needs form-to-type resolution at emit time, a layering question rather than a missing fact. Costs the 312 B of RM2 category 2 (`re.tur`'s `re-find-from`) |
...
| Report | Severity | One line |
| --- | --- | --- |
| [release-archive-cannot-compile](release-archive-cannot-compile.md) | high | An extracted release tarball can run `tur --version` and the REPL but not `tur run`. **Windows fixed** (a mangled `-L`, plus a prefix-layout archive); macos verified to fail the same way and linux inferred -- both still ship the flat layout |
