...
| [self-recursive-fn-returning-call-into-fat-sink](self-recursive-fn-returning-call-into-fat-sink.md) | medium-high | a fn-returning defn whose SELF-call result feeds a `^fat` parameter segfaults at runtime (the natural recursive-combinator shape); routing the same call through a one-line forwarder works, pointing at fat-normalization state on the self-binding |
