...
| [cli-mallocs-module-base-dir-through-the-borrowed-path](cli-mallocs-module-base-dir-through-the-borrowed-path.md) | low | `src/main.c:7827` and `:8578` malloc a directory slice and assign `env->module_base_dir` directly, which leaves `module_base_dir_owned` false, so `turi_env_free` correctly declines to free what it was told it does not own. `turi_env_set_module_base_dir` exists for this and strdups + owns; both sites hand-rolled the copy, presumably because they need a `strrchr` slice. A few dozen bytes once per process -- worth a row only because it fires on every `--interpret` run under `ASAN_OPTIONS=detect_leaks=1`, the channel CLAUDE.md documents for chasing real interpreter leaks, and sits where a real finding would be. The sibling `*args*` cons cells are NOT this bug (process-lifetime by design) |
