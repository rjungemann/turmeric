...
| [partial-head-ap-calls-fat-closure-as-thin-pointer](partial-head-ap-calls-fat-closure-as-thin-pointer.md) | high | **Filed 2026-09-25.** An `Applicative` instance over a partial head such as `(Result _ B)` or a user `(Either _ E)` gets no by-value `ap` spec, so compiled `ap` passes a by-value monomorph into the carrier body, which calls the `Right`/`Ok` fat closure as a thin C function pointer and segfaults; `--interpret` prints the right answer. `Option` is safe (full head, has the spec). Blocks adding `Applicative [(Result _ B)]` to the stdlib, and with it typeclass-superclasses-plan SC8b's `Monad` over `Applicative` |
