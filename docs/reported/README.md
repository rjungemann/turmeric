...
| [c2mir-llp64-long-long-vs-unsigned-int](c2mir-llp64-long-long-vs-unsigned-int.md) | medium | Filed 2026-09-26. c2mir's `arithmetic_conversion` asks `LONG_MAX` about every signed type of rank long or above, so on win64 (LLP64) `long long OP unsigned int` is computed in `unsigned int`: `(int64_t)5u - 7u` is 4294967294 under `tur jit`. Silent wrong answers. Fix written and tested (C11 6.3.1.8, plus a c-test) but it lives in the MIR fork and was not pushed; the full patch is in the report. `r7rs-bignums` was the visible victim and is worked around in-tree |
