...
| [typeclass-method-resolution-ignores-the-class](typeclass-method-resolution-ignores-the-class.md) | medium | Method calls resolve by name against the elaborated **instance** table, never the `defclass` or the caller's constraint (`elab_typeclasses.c:6675`). Two symptoms, one cause: (A) a `definstance` placed BELOW a constrained use fails with "no typeclass method found" for a method declared two lines above -- moving it up compiles, and no guide documents any ordering rule; (B) an UNCONSTRAINED generic body calling a class method passes `tur check` with rc=0 and then fails inside `cc` with a mangled-symbol type error, because the constraint is what selects the instance. Also corrects a stale claim in the archived ECB plan: the wrong-world case it calls "noticeably worse" now gives a good TUR-E0001 naming class, world, function and tyvar |
| [nested-class-method-call-picks-the-first-instance](nested-class-method-call-picks-the-first-instance.md) | high | **Silent wrong answer.** A NESTED class-method call inside a CONSTRAINED generic resolves the outer call to the first declared instance, not the inner call's result type: a `float` specialization emits `__inst_JS_join_int(__ps_276, y)` where `__ps_276` is a `double`, so `(join (join 2.5 7.1) 7.1)` prints `7`. Needs all three of constrained body + nesting + a non-first instance; flat calls and concrete types are correct, and `--interpret` answers `7.1`, so a turi cross-check HIDES it. Sibling of typeclass-method-resolution-ignores-the-class (whose symptoms are explicitly not wrong answers). `-Wfloat-conversion` on the emitted C flags exactly the bad line with zero noise -- cheap independent guard for the whole int-carrier-into-float-slot class. Blocks any Semigroup/Monoid/lattice vocabulary, where `combine : a -> a -> a` nests by construction |
| [nullary-class-method-unresolvable-over-newtype-tyvar](nullary-class-method-unresolvable-over-newtype-tyvar.md) | medium | A nullary class method (`(mempty [] : a)`) will not resolve against a type variable whose instances are `defopaque` newtypes: `(defn f [^Monoid A] [x : A] : A (mempty))` is `no instance 'Monoid tyvar'` even with a single instance. Needs BOTH halves -- nullary AND newtype-instanced; the same method resolves at a concrete newtype (`(defn g [] : Sum (mempty))`), and a non-nullary method (`combine`) resolves over the same newtype tyvar. Blocks the newtype-per-instance idiom (`Sum`/`Product`/`Min`/`Max`), which is the only way to give one carrier several algebras |
