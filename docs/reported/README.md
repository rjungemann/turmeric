...
| [nullary-generic-call-under-tyvar-expectation](nullary-generic-call-under-tyvar-expectation.md) | low-medium | Filed 2026-09-19. A nullary generic call as an argument whose expected type is tyvar-shaped -- `(make-struct W 8 (none))` for `(opt (Option A))`, `(wrap 3 (box-nil))` -- is rejected with `expected (type-app Option tyvar 'A'), got (type-app Option tyvar 'A')`: the callee's and the argument's fresh `A` are compared, not unified. Workaround: ascribe the instantiation. Not the self-typed shape's (reproduces on a non-recursive def) |
