...
| [closure-in-defdata-field](closure-in-defdata-field.md) | medium | a `:fn` field in a `defdata` accepts a capturing closure, type-checks, and SIGSEGVs when forced (the ascription calls a fat closure as a bare fn pointer); a non-capturing one works, so the shape you write first is fine and the shape you need crashes. `defopaque :ptr<void>` works and is undocumented. Blocks the lazy-stream fix |
