...
| Report | Severity | One line |
| --- | --- | --- |
| [turi-show-instance-with-inline-c-body-prints-a-pointer](turi-show-instance-with-inline-c-body-prints-a-pointer.md) | medium | **Filed 2026-09-09.** A typeclass named exactly `Show` with a method named exactly `show`, backed by inline-C, reaches neither of the interpreter's two correct outcomes -- no matcher claims it (`TUR_IC_TRACE` logs nothing) and the clean `inline-C not supported` guard never fires -- so `(println (.show 7.35))` prints the impl's raw carrier word, `88167088870656`, with no diagnostic. Renaming EITHER the class or the method restores the clean error; a pure Turmeric `Show` body is fine. Invisible to CI because `run-turi.sh` skips all 768 inline-C fixtures. Not Saffron and not S9 -- found as the control while checking S9's float dispatch. |
...
| Report | Severity | One line |
| --- | --- | --- |
| [jit-suite-reports-pass-when-the-engine-is-disabled](jit-suite-reports-pass-when-the-engine-is-disabled.md) | medium | **Filed 2026-09-07.** `run-jit.sh` counts a `TUR-W0070` cc fallback as a PASS (deliberately -- a fixture the engine cannot yet compile should not be red, and stamping it would stop a future engine improvement reclaiming it) and prints the aggregate fallback count without asserting anything about it. So the engine can go from compiling the whole tree to compiling NOTHING and the suite still prints `0 failed` and exits 0. That is not hypothetical: one `__auto_type` in the emitter (c2mir cannot parse it) put every program carrying an erasing ascription -- i.e. every program, the stdlib prelude has one -- on the cc path, and the only reason CI noticed was that the FALLBACK then failed for 10 fixtures on macOS for an unrelated reason, surfacing as `stdout mismatch (via cc fallback)` three steps from the cause. Fix direction: ratchet the fallback set the way `tests/repr-decision-baseline.txt` ratchets repr decisions -- NAMES rather than a count, which rots into a rubber-stamp as fixtures are added -- plus a one-second smoke assertion that a single trivial program compiles through the engine without falling back, which catches the tree-wide case with no list to maintain. Carries a second, still-open defect it was hiding: after an engine attempt fails, `cmd_jit`'s fallback into `cmd_run` emits calls to monomorph specializations (`over_px__lens_*`) it never defines, so the link fails -- reproduces on Linux, but only via `tur jit`, never `tur run`. And a THIRD: with the engine restored, `gc-heap-struct-rc` answers `0 / 1376` under a `-DTUR_DEBUG_SANITIZE=OFF` JIT build and `0 / 0` under CI's (sanitizers on) -- on `main` as well as here, natively, no fallback. The JIT runs the program in-process, so it shares `tur`'s allocator and the sanitizer changes what the collector sees as live: a GC fixture's answer turning on a build flag that should not touch it. Reproduce JIT failures with CI's configuration, not the ASan escape hatch |
...
| [jit-c2mir-implicit-decl-truncates-pointers](jit-c2mir-implicit-decl-truncates-pointers.md) | high | c2mir has no prototype for `strtok`, `strpbrk` or `memchr`, so their returned pointers come back cut to 32 bits -- non-NULL and unusable. Fixed by emitting prototypes; the list of three is a lower bound |
...
| [interp-inline-c-opaque-segv-in-any-reflection](interp-inline-c-opaque-segv-in-any-reflection.md) | medium (was **high**/crash) | **CRASH FIXED 2026-09-08.** `type-of` on an `any` holding an inline-C `defopaque` value segfaulted the interpreter: the inline-C result path re-tags a TURI_INT as TURI_STRUCT when the declared return is TY_ADT, and an opaque IS a TY_ADT, so payload 7 became a `TuriStruct *` of 0x7. Fixed by hardening the two readers that dereferenced it (alignment + zero page -- facts, not heuristics: neither can reject a real TuriStruct*). Direction 1, the "real fix", is BLOCKED and measured: `FnDef.return_type` carries no AdtDef -- NULL for an opaque AND for a genuine defdata -- because it exists for the lifetime pass and stores `type_from_kind(return_kind)`. Residue is a divergence, not a crash: interpreted answers `adt` where compiled answers `Route`; and an opaque over a LARGE int is still bit-indistinguishable from a pointer. Needed its own runner (`tur_interp_inline_c_opaque`) -- run-turi.sh PASS-skips every inline-C program, which is why nothing caught it |
| [any-drop-inlining-warns-free-nonheap](any-drop-inlining-warns-free-nonheap.md) | low | gcc warns `'free' called on a pointer to an unallocated object '7'` in EMITTED code when it inlines `__tur_any_drop` at a site whose `any` payload is a constant: the free is guarded by a runtime registry lookup that correctly says `boxed = 0`, but the optimiser cannot see through it. The program is correct; this is a scary-looking warning in a user's build on code they did not write. Fix is to skip the drop site entirely when the payload's boxed flag is statically 0, which the emitter already computes |
...
| [any-fn-widen-through-local-binding-leaks](any-fn-widen-through-local-binding-leaks.md) | low-medium | A fn widened to `any` through a LOCAL binding (`(let [f (fn ...)] (peek f))`) leaks its 24-byte fat shim box per widen -- unbounded in a loop. The two common shapes (a `defn` named as a value, a lambda widened directly) are leak-clean because their box is a link-time constant the emitter hoists to a static; a local binding is not one, and the guard cannot see through it to the lifted global. The frame-box rule would cover it in argument position but needs a non-retain mask bit that is 0 for an ordinary `any` parameter. Residue of the capturing-closure row above |
| [lang-unknown-base-diagnostic-names-nothing](lang-unknown-base-diagnostic-names-nothing.md) | low | An unknown `#lang` base reports "#lang unknown is not yet implemented" -- where "unknown" is `reader_type_name(READER_UNKNOWN)`, the enum's own name, not the user's token. Two conditions share one message (a base that does not exist, and one that exists but has no reader), and the second is what the wording describes while the first is what happens. The base token is not threaded to the report site, unlike the unknown-LAYER path which already has an out-param for exactly this. Pre-existing; noticed while adding four new base spellings in saffron S1 |
...
| [saffron-match-any-scrutinee-on-parametric-adt-emits-bad-c](saffron-match-any-scrutinee-on-parametric-adt-emits-bad-c.md) | medium | `(match s (Sq w) ...)` on an unannotated (`any`) Saffron scrutinee emits uncompilable C when the ADT is PARAMETRIC -- the emitter casts the 16-byte `tur_tagged_t` straight to a pointer because the `any`-scrutinee narrow declines on an explicit `n_type_params == 0` guard. **Filed first as a `defgadt` bug and that was wrong**: a parametric `defdata` fails identically and a non-parametric one works, so the axis is arity. **Relaxing the guard was ATTEMPTED and REVERTED**: it needs `kind_for_arity` (type_adt hardcodes KIND_STAR), and then the narrow fires and PANICS `any holds a different instantiation of Shape` -- the box holds what the CONSTRUCTOR built (`(Sq 6)` is a `(Shape int)`), unlike a container literal, which the S6 widen makes `(Vec any)`. Trading a compile error for a runtime panic is the wrong direction. Real fix is to widen a ctor payload to `any` in Saffron (direction 1), which also has to answer why an `any` arm binder used twice is a TUR-E0005 use-after-move. Distinct from the match-arm-binder report -- checked, not assumed |
| [cps-coloring-walk-has-no-arm-for-union-inject](cps-coloring-walk-has-no-arm-for-union-inject.md) | medium | `(defn go [] (call/cc (fn [k] (k 42))))` in a Saffron file aborts `tur: emit: EX_CALLCC reached the direct emitter`; annotating the return `: int` fixes it, and plain Turmeric is fine -- so the trigger is the `any` RETURN, not the dialect or `call/cc`. Probed rather than read: an `any` return wraps the body in `EX_UNION_INJECT` (body kind 100 vs 53 = `EX_CALLCC` when annotated), `cps_directly_uses_control` has no arm for it and takes `default: return false`, so `cps_colored` is 0, the candidate loop is gated on it, and `fn_sig_ok` is never even consulted (`slot_box_ty` admits `TY_ANY` -- the sig gate is innocent). The walk already has an `EX_ASCRIBE` arm with a comment describing exactly this situation. Same class as the fixed `expr_subtree_has_inline_c` gap. **PARTIALLY FIXED 2026-09-08:** the four coloring arms are landed and the repro STILL ICEs -- necessary, not sufficient. `EX_DYN_OP` and `EX_DYN_CALL` now have repros too (`EX_DYN_FIELD` does not -- its arm is by construction). With the arms in, the function is coloured and PASSES `fn_sig_ok`, then is evicted one stage later by the CPS IR lowering: `BODY-UNSUPPORTED ... EX_#100`. `cps_ir.c` handles the node only as DELEGATABLE (operand atomic or control-free), which a widen over a `call/cc` correctly fails. Remaining work is a real lowering -- either a new CPS primitive (must reproduce the widen's tag/`frame_box` decision, currently owned by the direct emitter) or hoisting the control op out, which needs Expr-from-CVar synthesis that does not exist. Neither attempted: a half-done change here yields a wrong answer, not a loud one |
| [cli-usage-error-paths-exit-zero](cli-usage-error-paths-exit-zero.md) | medium | An unknown flag is printed as a usage error and then EXITS 0, so `tur build --typo || exit 1` reports success. The `usage_*` helpers return 0 -- right for `--help`, which is a successful request -- and the error paths reuse the same helper for its output, inheriting the status. Checked on `repl` and `build`; both behave identically, so it is the shape rather than one site. Found while adding `tur repl --lang`, whose rejection test failed against it. Fix: keep `usage_*` for printing and give the error paths a status of their own; wants a per-subcommand test asserting `--help` exits 0 AND a bad flag does not, since either alone passes for the wrong reason |
| [gendocs-misparses-the-spaced-annotation-form](gendocs-misparses-the-spaced-annotation-form.md) | medium | `tools/gendocs.py` splits a parameter vector on whitespace, so the FUSED `[a :int]` parses right and the SPACED `[a : int]` does not: the parameter gets type `':'`, a PHANTOM parameter named `int` appears, and the return type is `':'`. Measured across stdlib: 1064 of 1999 definitions (53%) get `return_type == ':'`, 984 have a param typed `':'`, 114 files affected. Not an obscure spelling -- 612 stdlib defns use it and CLAUDE.md's own style guide writes it throughout. LATENT, not shipped: `docs/api/` is empty, so it would corrupt the reference on the next `tur run docs`. Found while checking whether gendocs needed Saffron work; it does NOT -- untyped params parse correctly, which is the S8 answer. Measured by loading `parse_tur_file` with a stub, since gendocs cannot run without the `markdown` package; the rendered HTML was not inspected |
...
| [set-of-element-type-is-not-checked](set-of-element-type-is-not-checked.md) | medium | `(set-of 1 "two")` type-checks as `(Set int)` and passes to a `(Set int)` parameter while holding a cstr pointer -- plain Turmeric, no `any`, both back ends. `vec-of` routes elements through `tur-vec-homog__` and `hamt-of` through `tur-map-homog__`; `set-of` has no counterpart, so each `set-add1` resolves `hash`/`mk-box`/`mk-cmp` at its OWN element's type. That is why the runtime is right -- content-keyed dedup, membership by value -- and only the exported type is a lie. **Sequencing CORRECTED 2026-09-08**: the missing `tur-set-homog__` twin cannot land alone. `tests/fixtures/saffron-set-literal`, written after this report, pins a heterogeneous `#set{1 "two" 7.1}` working on both back ends -- which the check would turn into a diagnostic, trading a type lie for a lost capability. A set has no honest heterogeneous type until `any` is one (`(Set any)` refuses for want of `Hash[any]`/`MapKey[any]`), so it is admit-`any` FIRST, then the check relaxed for `any` -- the shape Vec and Map already have |
