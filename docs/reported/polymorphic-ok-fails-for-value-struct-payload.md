...
status: OPEN. Filed 2026-06-13 at the end of the P2a typed Decode follow-up. Workaround in tree: the macro-driven Decode side of `derive-json` (in `../turmeric-spices/spices/json/src/json/encode.tur`) is plumbed but not emitted -- the macro helpers `__decode-field`, `__decode-fields`, `__decode-make-struct` exist and are exported, but `derive-json` emits the Encode side only. Hand-written `(definstance Decode [T] (decode [doc val] ```c ... return tur_ok((int64_t)(intptr_t)heap_user); ```))` for user defstruct types works fine; the gap is purely in the polymorphic constructor's surface.
---
