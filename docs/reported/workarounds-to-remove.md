...
## ~~4. `with-untrailed` callers bind a result they do not want~~ -- DONE

Removed 2026-08-26, in the same change that fixed
[poly-call-in-statement-position-dropped](../archive/poly-call-in-statement-position-dropped.md).
The HAZARD block is gone from `with-untrailed`'s docstring, its example is the
natural discarded-result form again, and `sx2-trail-combinators` discards rather
than binds. Kept here struck through rather than deleted: the point of this file
is that a workaround leaves a trace, and so should its removal.
