...
(A) and (D) are one defect (non-capturing / apparently-non-capturing closure at
a fat-handle boundary) and are the ones to take first; (D) additionally needs
capture analysis to see through a nested lambda. (B) is a lifetime bug in the
drop glue for a captured fat handle. (C) is a missing env thread-through.
