...
8. `upcoming/bare-fat-lambda-param-plan.md`
   ...
   - Deps: 6, 7
   - Status: Proposed (found 2026-06-03 while wiring closure-returning instance method retype)
...
10. `reported/ascribe-captured-var-in-closure.md`
    - Kind: report
    - Goal: `(:: v :T)` on a closure-captured `v` emits the raw local name instead of the env access (`__env_*->v`) -- generated C fails to compile
    - Deps: independent
    - Status: Open
...
13. `reported/fn-typed-return-lowered-to-result-type.md`
    ...
    - Deps: independent (surfaced by 15 / G2 spike)
    - Status: Open

## Phase 2 -- Type system expansion

14. `upcoming/sum-types-either-plan.md`
    - Kind: plan
...
15. `upcoming/language-readiness-for-typed-signal-plan.md`
    - Kind: plan
...
16. `upcoming/stdlib-opaque-handle-types-plan.md`
    - Kind: plan
    - Goal: Wrap bare `:int`/`:ptr<void>` resource handles in `defopaque` newtypes
    - Deps: 1
    - Status: In Progress

17. `upcoming/stdlib-inline-c-deworkaround-plan.md`
    - Kind: plan
...
18. `upcoming/stdlib-advanced-typing-plan.md`
    - Kind: plan
...
19. `upcoming/stdlib-arrow-scaleback-plan.md`
    - Kind: plan
...
20. `upcoming/stdlib-type-erasure-cleanup-plan.md`
    - Kind: plan
    - Goal: Replace int64-erased typeclass stubs with real instances
    - Deps: 6, 14 (plus operator-name mangling fix tracked inside the plan)
    - Status: Draft (Phases A-C with subtasks defined, not implemented)

## Phase 6 -- Typeclass reintroduction

21. `upcoming/stdlib-arrow-typeclass-reintroduction-plan.md`
    - Kind: plan
...
22. `reported/signal-spice-broken-build.md`
    - Kind: report
    ...
    - Deps: independent (prerequisite finding for 23)
    - Status: Open (severity: high)

23. `upcoming/tur-signal-rebuild-plan.md`
    - Kind: plan
    - Goal: Rebuild the tur-signal spice on top of the modern typed infrastructure
    - Deps: 15, 19, 22
    - Status: Draft, blocked on plan 15 (Phases 1-6 designed)
