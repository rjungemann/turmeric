...
static void test_try_catch_finally() {
    ExceptionHandler __t4;
    __t4.parent = global_handler_chain;
    global_handler_chain = &__t4;
    if (setjmp(__t4.jmp_buf) == 0) {
        global_handler_chain = global_handler_chain->parent;
        goto __t5;
    } else {
        if (tur_exception_matches(__t4.caught, 0)) {
            void *e = __t4.caught->payload;
            tur_exception_free(__t4.caught);
            global_handler_chain = global_handler_chain->parent;
            goto __t5;
    } else {
        global_handler_chain = global_handler_chain->parent;
