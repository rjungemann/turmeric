...
static int64_t test_typed_catch_int();
static const char * test_typed_catch_cstr();
...
static const char * test_typed_catch_cstr() {
    const char * __t4 = (const char *)0;
    ExceptionHandler __t3;
    __t3.caught = NULL;
    __t3.parent = global_handler_chain;
    global_handler_chain = &__t3;
    if (setjmp(__t3.jmp_buf) == 0) {
        tur_throw(13, (void*)"error", 11, "<unknown>");
        global_handler_chain = global_handler_chain->parent;
    } else {
        if (tur_exception_matches(__t3.caught, 13)) {
            const char * e_24 = (const char *)__t3.caught->payload;
            __t4 = e_24;
            tur_exception_free(__t3.caught);
            global_handler_chain = global_handler_chain->parent;
    } else {
        global_handler_chain = global_handler_chain->parent;
        longjmp(global_handler_chain->jmp_buf, 1);
