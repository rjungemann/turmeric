...
static void test_typed_catch_int();
static void test_typed_catch_cstr();
static void test_typed_catch_int() {
        ...
        if (setjmp(__t0.jmp_buf) == 0) {
            ...
            *__t1 = INT64_C(42);
            tur_throw(3, (void*)__t1, 5, "<unknown>");
            global_handler_chain = global_handler_chain->parent;
        } else {
            if (tur_exception_matches(__t0.caught, 3)) {
                void *e = __t0.caught->payload;
                tur_exception_free(__t0.caught);
                global_handler_chain = global_handler_chain->parent;
        } else {
            global_handler_chain = global_handler_chain->parent;
            longjmp(global_handler_chain->jmp_buf, 1);
        }
    }
}

static void test_typed_catch_cstr() {
    ExceptionHandler __t2;
    __t2.parent = global_handler_chain;
    global_handler_chain = &__t2;
    if (setjmp(__t2.jmp_buf) == 0) {
        tur_throw(5, (void*)"error", 11, "<unknown>");
        global_handler_chain = global_handler_chain->parent;
    } else {
        if (tur_exception_matches(__t2.caught, 5)) {
            void *e = __t2.caught->payload;
            tur_exception_free(__t2.caught);
            global_handler_chain = global_handler_chain->parent;
    } else {
        global_handler_chain = global_handler_chain->parent;
        longjmp(global_handler_chain->jmp_buf, 1);
