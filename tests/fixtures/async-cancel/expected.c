...
static bool fc_cancelled_(void * f) {
        typedef struct { pthread_mutex_t lock; pthread_cond_t ready; int64_t value; int64_t exn; bool is_set; bool is_ok; } FutureCell;
  FutureCell *fc = (FutureCell *)f;
  pthread_mutex_lock(&fc->lock);
...
int main() {
        int64_t __t0;
        {
            void * fc_217 = fc_new();
            (void)fc_217;
            if (fc_done_((void *)(intptr_t)(fc_217))) {
                puts("done");
            } else {
                puts("pending");
            }
            fc_cancel((void *)(intptr_t)(fc_217));
            if (fc_cancelled_((void *)(intptr_t)(fc_217))) {
                puts("cancelled");
            } else {
                puts("FAIL");
            }
            fc_free((void *)(intptr_t)(fc_217));
            int64_t __t1;
            __t1 = INT64_C(0);
            __t0 = __t1;
        }
