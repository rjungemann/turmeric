...
static void * u_ok(int64_t);
static void * u_err(int64_t);
static bool u_ok_(void *);
static bool u_err_(void *);
static int64_t u_ok_val(void *);
static int64_t u_err_val(void *);
static int64_t u_result_unwrap(void *);
static int64_t u_result_unwrap_or(void *, int64_t);
static int64_t u_result_expect(void *, const char *);
...
static void * u_ok(int64_t x) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = malloc(sizeof(*result));
  result->is_ok = true; result->ok_val = x; result->err_val = 0; return result;
  
}

static void * u_err(int64_t e) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = malloc(sizeof(*result));
  result->is_ok = false; result->ok_val = 0; result->err_val = e; return result;
  
}

static bool u_ok_(void * r) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  return result != NULL && result->is_ok;
  
}

static bool u_err_(void * r) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  return result == NULL || !result->is_ok;
  
}

static int64_t u_ok_val(void * r) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  return result->ok_val;
  
}

static int64_t u_err_val(void * r) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  return result->err_val;
  
}

static int64_t u_result_unwrap(void * r) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  if (result == NULL || !result->is_ok) { fprintf(stderr, "u-result-unwrap: u-err\n"); abort(); }
  return (int)result->ok_val;
  
}

static int64_t u_result_unwrap_or(void * r, int64_t default_val) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
  if (result != NULL && result->is_ok) return (int)result->ok_val;
  return default_val;
...
static int64_t u_result_expect(void * r, const char * msg) {
        struct { bool is_ok; int64_t ok_val; int64_t err_val; } *result = (void*)r;
...
int main() {
        int64_t __t0;
        {
            void * r1_636 = u_ok(INT64_C(42));
            (void)r1_636;
            int64_t __t1;
            {
                void * r2_637 = u_err(INT64_C(7));
                (void)r2_637;
                puts((u_ok_((void *)(intptr_t)(r1_636))) ? "true" : "false");
                puts((u_ok_((void *)(intptr_t)(r2_637))) ? "true" : "false");
                puts((u_err_((void *)(intptr_t)(r1_636))) ? "true" : "false");
                puts((u_err_((void *)(intptr_t)(r2_637))) ? "true" : "false");
                printf("%lld\n", (long long)(u_ok_val((void *)(intptr_t)(r1_636))));
                printf("%lld\n", (long long)(u_err_val((void *)(intptr_t)(r2_637))));
                printf("%lld\n", (long long)(u_result_unwrap((void *)(intptr_t)(r1_636))));
                printf("%lld\n", (long long)(u_result_unwrap_or((void *)(intptr_t)(r2_637), INT64_C(99))));
                printf("%lld\n", (long long)(u_result_expect((void *)(intptr_t)(r1_636), "should not abort")));
                int64_t __t2;
                __t2 = INT64_C(0);
                __t1 = __t2;
            }
