...
int main() {
        {
            IoError io_err_211 = (IoError){.message = "file not found", .errno_val = INT64_C(2)};
            (void)io_err_211;
            puts("IoError created");
        }
        {
            ParseError parse_err_212 = (ParseError){.message = "syntax error", .line = INT64_C(10), .col = INT64_C(5), .file = "test.tur"};
            (void)parse_err_212;
            puts("ParseError created");
        }
        {
            Error err_213 = (Error){.message = "base error", .cause = INT64_C(0)};
            (void)err_213;
            puts("Error created");
        }
        int64_t __t0;
