...
static void * fc_get(void *);
static bool fc_done_(void *);
static void fc_free(void *);
static bool result_ok_(void *);
static int64_t result_val(void *);
static int64_t result_err(void *);
static void result_free(void *);
static void * future_worker(void *);
static void * make_worker_arg(void *, int64_t);
static void * thread_spawn_raw(void *, void *);
static void thread_join_raw(void *);
static void print_labelled(const char *, int64_t);
...
static void fc_fulfill(void * fc, int64_t v) {
...
static void fc_fail(void * fc, int64_t e) {
        typedef struct { pthread_mutex_t lock; pthread_cond_t ready; int64_t value; int64_t exn; bool is_set; bool is_ok; } FutureCell;
  FutureCell *cell = (FutureCell *)fc;
  pthread_mutex_lock(&cell->lock);
  if (cell->is_set) { pthread_mutex_unlock(&cell->lock); return; }
...
static bool fc_done_(void * fc) {
        typedef struct { pthread_mutex_t lock; pthread_cond_t ready; int64_t value; int64_t exn; bool is_set; bool is_ok; } FutureCell;
  FutureCell *cell = (FutureCell *)fc;
  pthread_mutex_lock(&cell->lock);
  bool done = cell->is_set;
  pthread_mutex_unlock(&cell->lock);
  return done;
  
}

static void fc_free(void * fc) {
        typedef struct { pthread_mutex_t lock; pthread_cond_t ready; int64_t value; int64_t exn; bool is_set; bool is_ok; } FutureCell;
  FutureCell *cell = (FutureCell *)fc;
...
static int64_t result_val(void * r) {
        typedef struct { bool is_ok; int64_t ok_val; int64_t err_val; } Result;
  return (int)((Result *)r)->ok_val;
  
}

static int64_t result_err(void * r) {
        typedef struct { bool is_ok; int64_t ok_val; int64_t err_val; } Result;
...
static void * future_worker(void * arg) {
  ...
  int64_t val = (int64_t)(uintptr_t)args[1];
...
static void * make_worker_arg(void * cell, int64_t val) {
...
static void * thread_spawn_raw(void * fn_ptr, void * arg) {
...
static void thread_join_raw(void * t) {
        pthread_join(*(pthread_t *)t, NULL);
  free(t);
  
}

static void print_labelled(const char * label, int64_t val) {
        printf("%s %lld\n", label, (long long)val);
...
int main() {
        int64_t __t0;
        {
            ...
            (void)fc_247;
            {
                void * arg_248 = make_worker_arg((void *)(intptr_t)(fc_247), INT64_C(42));
                (void)arg_248;
                {
                    void * t_249 = thread_spawn_raw((void *)(intptr_t)(future_worker), (void *)(intptr_t)(arg_248));
                    (void)t_249;
                    {
                        void * result_250 = fc_get((void *)(intptr_t)(fc_247));
                        (void)result_250;
                        if (result_ok_((void *)(intptr_t)(result_250))) {
                            print_labelled("ok:", result_val((void *)(intptr_t)(result_250)));
                        } else {
                            print_labelled("err:", result_err((void *)(intptr_t)(result_250)));
                        }
                        result_free((void *)(intptr_t)(result_250));
                        thread_join_raw((void *)(intptr_t)(t_249));
                    }
                    fc_free((void *)(intptr_t)(fc_247));
                }
            }
            int64_t __t1;
            __t1 = INT64_C(0);
