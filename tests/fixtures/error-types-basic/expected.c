...
int main() {
        {
            void * ve_25 = ValidationError_("must be positive", "age");
            (void)ve_25;
            puts(ValidationError_message(ve_25));
            puts(ValidationError_field(ve_25));
            ValidationError_free(ve_25);
        }
        {
            void * ne_26 = NotFoundError_("config.toml");
            (void)ne_26;
            puts(NotFoundError_what(ne_26));
            NotFoundError_free(ne_26);
        }
        {
            void * pe_27 = PermissionError_("read denied", "/etc/shadow");
            (void)pe_27;
            puts(PermissionError_message(pe_27));
            puts(PermissionError_path(pe_27));
            PermissionError_free(pe_27);
        }
        int64_t __t0;
        __t0 = INT64_C(0);
        return (int)__t0;
