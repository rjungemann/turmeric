tests/fixtures/lint-unsafe-size/input.tur:2:3: warning: unsafe block has 5 expressions (threshold: 2); consider breaking into smaller blocks
1 | (defn main [] :int
2 |   (unsafe
  |   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
3 |     (raw-malloc 8)
...
int main() {
        ...
        *__eff_chain_1 = &__eff_frame_1;
        (void)(malloc(INT64_C(8)));
        (void)(malloc(INT64_C(8)));
        (void)(malloc(INT64_C(8)));
        free(malloc(INT64_C(8)));
        int64_t __t3;
        __t3 = INT64_C(0);
        int64_t __t2 = __t3;
