tests/fixtures/lint-unsafe-doc/input.tur:2:3: warning: unsafe block requires a ;;; SAFETY: comment documenting why unsafe code is needed
1 | (defn main [] :int
2 |   (unsafe
  |   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
3 |     (let [x 42]
