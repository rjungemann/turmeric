...
int main() {
        gc_enable();
        {
            int64_t i_261 = INT64_C(0);
            (void)i_261;
            while (1) {
                if (!(((i_261) < (INT64_C(20))))) break;
                {
                    int64_t *__t0 = (int64_t *)malloc(sizeof(int64_t));
                    *__t0 = i_261;
                    RcControlBlock *__t1 = rc_cb_alloc(0, 3, NULL);
                    __t1->value = __t0;
                    RcControlBlock * r_262 = __t1;
                    (void)r_262;
                    {
                        RcControlBlock *__t2 = r_262; rc_weak_increment(r_262);
                        RcControlBlock * w_263 = __t2;
                        (void)w_263;
                        rc_strong_decrement(r_262);
                        rc_free_queue_drain();
                        gc_force();
                    }
                }
                i_261 = ((i_261) + (INT64_C(1)));
            }
        }
        printf("%lld\n", (long long)(INT64_C(1)));
        int64_t __t3;
