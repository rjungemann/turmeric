...
int main() {
        gc_enable();
        {
            int64_t i_208 = INT64_C(0);
            (void)i_208;
            while (1) {
                if (!(((i_208) < (INT64_C(20))))) break;
                {
                    int64_t *__t0 = (int64_t *)malloc(sizeof(int64_t));
                    *__t0 = i_208;
                    RcControlBlock *__t1 = rc_cb_alloc(0, 3, NULL);
                    __t1->value = __t0;
                    RcControlBlock * r_209 = __t1;
                    (void)r_209;
                    {
                        RcControlBlock *__t2 = r_209; rc_weak_increment(r_209);
                        RcControlBlock * w_210 = __t2;
                        (void)w_210;
                        rc_strong_decrement(r_209);
                        rc_free_queue_drain();
                        gc_force();
                    }
                }
                i_208 = ((i_208) + (INT64_C(1)));
            }
        }
        printf("%lld\n", (long long)(INT64_C(1)));
        int64_t __t3;
