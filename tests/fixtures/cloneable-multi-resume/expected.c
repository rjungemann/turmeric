...
typedef struct { int64_t k; } __clenv_0;
static void *__clenv_0_clone(const void *src) {
    __clenv_0 *copy = malloc(sizeof(__clenv_0));
    if (!copy) abort();
    const __clenv_0 *s = (const __clenv_0 *)src;
    copy->k = s->k;
    return copy;
}
static void __clenv_0_drop(void *p) { free(p); }

int main() {
        {
            __clenv_0 *__t1 = malloc(sizeof(__clenv_0));
            if (!__t1) abort();
            __t1->k = k_198;
            tur_cloneable_cont *__t2 = tur_cloneable_cont_alloc(__cont_fn_0, __t1, __clenv_0_clone, __clenv_0_drop);
            int64_t __t3 = k_identity((int64_t)(intptr_t)__t2);
            int64_t k_198 = __t3;
            (void)k_198;
            printf("%lld\n", (long long)(tur_cloneable_cont_resume(tur_cloneable_cont_clone(k_198), INT64_C(10))));
            printf("%lld\n", (long long)(tur_cloneable_cont_resume(k_198, INT64_C(20))));
        }
        int64_t __t4;
        __t4 = INT64_C(0);
        return (int)__t4;
