...
#
# Drives `tur --enable=jit repl` inside fixture spices and asserts the
...
probe=$("$TUR_BIN" --enable=jit jit 2>&1 || true)
case "$probe" in
  *"usage: tur jit"*) ;;
...
run() {
    (cd "$PROJ" && printf '%b' "$1" | "$TUR_BIN" --enable=jit repl 2>&1)
}
...
EOF
out=$(cd "$P2" && echo ':quit' | "$TUR_BIN" --enable=jit repl 2>&1)
...
mkfifo "$FIFO"
OUT4="$P2/out.log"
(cd "$P2" && "$TUR_BIN" --enable=jit repl < "$FIFO") >"$OUT4" 2>&1 &
REPL_PID=$!
exec 3>"$FIFO"
sleep 1
cat > "$P2/src/sh.tur" <<'EOF'
