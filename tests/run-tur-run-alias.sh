...
OUT="$("$TUR" run hi 2>&1)"
RC=$?
if [ "$RC" -eq 0 ] && echo "$OUT" | grep -q "hello from greet"; then
...
RC2=$?
if [ "$RC2" -eq 0 ] && echo "$OUT2" | grep -q "after-alias reached"; then
...
RC3=$?
if [ "$RC3" -eq 0 ] && echo "$OUT3" | grep -q "hello from greet"; then
