 ...
 * accessors.  source hash 1bea53f65b56e2f7. */
...
const char *g_tur_argv0 = "";  /* *argv0*: the program's own name, argv[0] (set in main) */
...
int tur_dk_pinned = 0;
void dk_free(DK *k) { if (tur_dk_pinned) return; while (k) { DK *n = k->borrow_next ? NULL : k->next; if (k->env_drop) k->env_drop(k->env); free(k); k = n; } }
...
__attribute__((unused)) void dk_free_node(DK *k) { if (tur_dk_pinned) return; if (k && k->env_drop) k->env_drop(k->env); free(k); }
...
tur_jmp_buf *g_dk_driver = NULL;      /* current entry-driver landing (NULL => inline) */
...
TUR_THREAD_LOCAL tur_escape_cont **tur_escape_live;
TUR_THREAD_LOCAL int tur_escape_live_n, tur_escape_live_cap;
...
#endif
int64_t tur_session_f64_bits(double d) { int64_t i; memcpy(&i, &d, sizeof i); return i; }
double tur_session_bits_f64(int64_t i) { double d; memcpy(&d, &i, sizeof d); return d; }
int64_t tur_session_f32_bits(float f) { int32_t i; memcpy(&i, &f, sizeof i); return (int64_t)i; }
float tur_session_bits_f32(int64_t i) { int32_t j = (int32_t)i; float f; memcpy(&f, &j, sizeof f); return f; }
typedef struct {
    pthread_mutex_t mu;
    pthread_cond_t  cv;
    int64_t val;
