...
"    return true;\n"
"}\n"
"/* E2: direct-fn-ptr -> cps-fn-ptr registry.  A colored function registers its\n"
" * {direct entry, __cps entry} pair; an indirect call to an EFFECTFUL fn-value in\n"
" * a DK-emitted body looks up the callee's __cps entry and threads __kont so the\n"
" * effect performed inside reaches the caller's handler, instead of calling the\n"
" * direct entry (own root prompt -> `unhandled effect`).  A pure/external callee is\n"
" * absent from the table and the caller falls back to the direct call. */\n"
"typedef struct { int64_t direct; int64_t cps; } __tur_cps_ent;\n"
"static __tur_cps_ent *__tur_cps_reg = NULL;\n"
"static size_t __tur_cps_reg_n = 0, __tur_cps_reg_cap = 0;\n"
"static void __tur_cps_register(int64_t direct, int64_t cps) {\n"
"    if (__tur_cps_reg_n == __tur_cps_reg_cap) {\n"
"        __tur_cps_reg_cap = __tur_cps_reg_cap ? __tur_cps_reg_cap * 2 : 16;\n"
"        __tur_cps_reg = (__tur_cps_ent *)realloc(__tur_cps_reg, __tur_cps_reg_cap * sizeof(__tur_cps_ent));\n"
"    }\n"
"    __tur_cps_reg[__tur_cps_reg_n].direct = direct;\n"
"    __tur_cps_reg[__tur_cps_reg_n].cps = cps; __tur_cps_reg_n++;\n"
"}\n"
"static int64_t __tur_cps_lookup(int64_t direct) {\n"
"    for (size_t i = 0; i < __tur_cps_reg_n; i++)\n"
