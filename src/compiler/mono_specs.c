...
/* Register (dedup by content hash) one resolved CONCRETE spec key. */
static void mono_concrete_register(const char *lens, const char *functor,
                                   const char *focus, const char *whole) {
    if (g_n_concrete >= MONO_SPEC_MAX) return;
    MonoConcreteKey k;
    memset(&k, 0, sizeof k);
    field_copy(k.lens, lens);
    field_copy(k.functor, functor);
    field_copy(k.focus, focus);
    field_copy(k.whole, whole);
...
static void resolve_walk(const Expr *e, const ResolveCtx *rc) {
    ...
    if (e->kind == EX_CALL) {
        ...
        if (fb && fb->name && fb->name->name &&
            ...
            if (lens)
                mono_concrete_register(lens, rc->functor, rc->focus, rc->whole);
        }
    }
    switch (e->kind) {
        case EX_PROGRAM:
...
void mono_specs_resolve_program(const void *prog_) {
    ...
    for (size_t i = 0; i < g_n_specs; i++) {
        ...
        ResolveCtx rc = { k->enclosing, (uint32_t)lens_idx,
