...
static const DiagExplanation diag_explanations_[] = {
    ...
    { TUR_E0715_NICHE_ELEMENT_ERASED_COMPARATOR,
      ...
      "\n"
      "`vec-eq?` is the one stdlib helper that hands its comparator RAW Vec\n"
      "slot words -- its inline-C loop passes a->data[i] straight through --\n"
      "rather than values that have crossed the carrier boundary.  A niche\n"
      ...
      "words and compiles correctly.  A NAMED function cannot be: it is\n"
      "elaborated once and the same function may also be called with genuine\n"
      "boxes, so its erased parameters have no decidable convention here.\n"
      "\n"
