...
typedef struct SL {
    ...
    const Symbol *p_eqv, *p_list, *p_length, *p_list_ref, *p_list_tail,
                 ...
                 *p_list_to_vector, *p_char, *p_big, *p_ratio, *s_cond_expand, *s_export,
 ...
 * and an exact non-integer its `(r7rs-ratio__ "<n>/<d>")`, data in the same
...
static bool is_char_form(SL *sl, const Form *f) {
    ...
    return (is_sym(h, sl->p_char) && v->tag == F_INT) ||
           ((is_sym(h, sl->p_big) || is_sym(h, sl->p_ratio)) && v->tag == F_STR);
}
